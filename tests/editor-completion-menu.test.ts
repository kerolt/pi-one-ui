...
describe("completion menu surfaces", () => {
  it("renders a transparent full-width palette shell without a results header or native count row", () => {
    ...
    expect(rows.slice(0, 2).every((row) => visibleWidth(row) === 40)).toBe(
      true,
    );
  });
  ...
  it.each([
    ...
    "leaves a selected-looking row with leading %s unchanged",
    (_name, line) => {
  ...
  it("omits only a trailing standalone native count row", () => {
    ...
    expect(
      isNativeCompletionCountRow(
        "\x1b]8;;https://example.com\x07(6/47)\x1b]8;;\x07",
      ),
    ).toBe(false);
    expect(
      omitTrailingNativeCompletionCountRow([
    ...
    expect(
      omitTrailingNativeCompletionCountRow(["  item", "  (1/47) details"]),
    ).toEqual(["  item", "  (1/47) details"]);
  });
