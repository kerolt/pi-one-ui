...
import {
  installToolGrouping,
  ToolGroupComponent,
} from "../extensions/transcript/renderer/tool/grouping.ts";
...
test("truncated tool summary remains clickable and highlights on hover", async () => {
  ...
  assert.equal(
    tool.expanded,
    false,
    "summary text and row padding are not clickable",
  );
...
test("parenthesized rich diff hint highlights and expands on click", async () => {
  ...
  try {
    ...
    assert.equal(
      renderRequests,
      1,
      "hover requests a repaint for white hint text",
    );
...
test("expanded tool group show-more opens preview instead of collapsing the group", () => {
  ...
  try {
    ...
    assert.equal(
      inputHandler?.(`\x1b[<0;${col};${showMoreRow + 1}M`)?.consume,
      true,
    );
    assert.equal(
      group.expanded,
      beforeExpanded,
      "show-more must not collapse the group",
    );
...
test("native mode hits the visible identical tool, not the offscreen duplicate", () => {
  ...
  try {
    ...
    assert.deepEqual(inputHandler?.(`\x1b[<0;${hintCol};2M`), {
      consume: true,
    });
...
test("native mode hits offset columns after parent layout prefix", async () => {
  ...
  try {
    ...
    assert.equal(
      renderRequests,
      1,
      "offset columns hover the final painted hint",
    );
    assert.deepEqual(inputHandler?.(`\x1b[<0;${offsetCol};2M`), {
      consume: true,
    });
    assert.equal(tool.expanded, true);
  } finally {
  ...
  try {
    ...
    try {
      ...
      assert.equal(
        renderRequests,
        rendersAfterOff,
        "off mode: hover has no effect",
      );
      tui.handleInput(`\x1b[<0;${col};${row}M`);
      assert.equal(
        expandedToolId,
        null,
        "off mode: tool click does not expand",
      );
      ...
      assert.equal(
        expandedToolId,
        "tool-1",
        "on mode: tool click expands again",
      );
    } finally {
