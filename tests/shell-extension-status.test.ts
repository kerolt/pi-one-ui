...
} from "../extensions/shell/extension-status";
...
describe("sanitizeExtensionStatusOriginalText", () => {
  it("preserves SGR color while stripping unsafe control sequences", () => {
    expect(
      sanitizeExtensionStatusOriginalText("\x1b[31mred\x1b[0m\nnext\tline"),
    ).toBe("\x1b[31mred\x1b[0m next line");
    expect(
      sanitizeExtensionStatusOriginalText("\x1b]133;A\x07prompt\x1b]133;B\x07"),
    ).toBe("prompt");
    expect(
      sanitizeExtensionStatusOriginalText("\x1b[32mok\x1b[0m\x1b[2K"),
...
describe("collectExtensionStatusSegments", () => {
  it("routes active statuses by placement and defaults unsaved keys to right", () => {
    ...
    expect(segments.right.map((segment) => segment.key)).toEqual([
      "gamma",
      "unsaved",
    ]);
    expect(
      [...segments.left, ...segments.middle, ...segments.right].map(
        (segment) => segment.key,
      ),
  ...
  it("sorts each placement alphabetically and skips sanitized-empty statuses", () => {
    ...
    expect(segments.left.map((segment) => segment.key)).toEqual([
      "alpha",
      "zeta",
    ]);
  ...
  it("treats prototype-shaped status keys as opaque strings", () => {
    ...
    const defaults = collectExtensionStatusSegments(
      statuses,
    ...
    expect(configured.left[0]).toMatchObject({
      key: "constructor",
      colorMode: "original",
    });
    expect(configured.middle[0]).toMatchObject({
      key: "toString",
      colorMode: "original",
    });
  ...
  it("falls back safely after runtime enum mutation", () => {
    const config = configWithExtensionStatuses({});
    const mutable = config.components.footer.styles.starship
      .extensionStatuses as unknown as {
