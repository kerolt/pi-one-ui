...
function toolCallMsg(
  timestamp: number,
  tools: Array<{ id: string; name: string; args?: any }>,
) {
  return {
    role: "assistant",
    timestamp,
...
test("on → compact reshapes to summary; compact → on restores tool cards", () => {
  ...
  try {
    ...
    expect(renderText(bash).length > 0).toBeTruthy();
    expect(!renderText(a1).some((l) => /Ran for .*bash×/.test(l))).toBeTruthy();

    // → compact：对齐 docs compact
    switchMode("compact", hooks, root);
    expect(renderText(a1).join("\n")).toMatch(
      /^Ran for 9s, bash×1, read×2, grep×1/,
    );
    expect(renderText(a1).join("\n")).toMatch(/click to show more/);
    expect(renderText(bash)).toStrictEqual([]);
    expect(renderText(edit).join("\n")).toMatch(/edit sample\.ts \(\+1 -1\)/);
    expect(renderText(aFinal).join("\n")).toMatch(/task done/);

    // → on：工具卡恢复，摘要消失
    switchMode("on", hooks, root);
    expect(renderText(bash).length > 0).toBeTruthy();
    expect(
      !renderText(a1).some((l) => /Ran for 9s, bash×1/.test(l)),
    ).toBeTruthy();
...
test("panel empty-root scan keeps live tracking (no /reload required)", () => {
  ...
  try {
    const msg = toolCallMsg(1, [
      { id: "b1", name: "bash", args: { command: "x" } },
    ]);
    const a1 = new AssistantMessageComponent(msg, true) as any;
    a1.updateContent(msg);
...
test("round accumulation survives on → compact switch", () => {
  ...
  try {
    const m1 = toolCallMsg(1, [
      { id: "b1", name: "bash", args: { command: "one" } },
    ]);
    const m2 = toolCallMsg(2, [
      { id: "f1", name: "fffind", args: { pattern: "x" } },
    ]);
    ...
    switchMode("compact", hooks, root);
    expect(renderText(a1).join("\n")).toMatch(/2s, bash×2, fffind×1, read×1/);
    expect(renderText(a2)).toStrictEqual([]);
    expect(renderText(a3)).toStrictEqual([]);
  } finally {
    config.mode = previous;
    hooks.shutdown();
  }
...
test("messages created while mode=on stay tracked for later compact switch", () => {
  ...
  try {
    ...
    const msg = toolCallMsg(10, [
      { id: "g1", name: "grep", args: { pattern: "z" } },
    ]);
    const a1 = new AssistantMessageComponent(msg, true) as any;
    a1.updateContent(msg);
    ...
    expect(renderText(grep).length > 0).toBeTruthy();
    expect(!renderText(a1).some((l) => /grep×1/.test(l))).toBeTruthy();
    ...
    expect(renderText(a1).join("\n")).toMatch(/1s, grep×1/);
    expect(renderText(grep)).toStrictEqual([]);
  } finally {
    config.mode = previous;
    hooks.shutdown();
  }
