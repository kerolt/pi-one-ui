...
test("collapsed thinking previews memoize complete output until invalidated", () => {
  ...
  try {
    ...
    expect(preview.render(12), "same state reuses complete output").toBe(first);

    expect(preview.render(16), "width changes recompute output").not.toBe(
      first,
    );
    ...
    ).not.toBe(beforeConfigChange);

    const beforeHover = preview.render(16);
    preview.setHintHovered(true);
    expect(preview.render(16), "hover changes recompute output").not.toBe(
      beforeHover,
    );

    const beforeInvalidation = preview.render(16);
    preview.invalidate();
    expect(preview.render(16), "invalidation clears memo").not.toBe(
      beforeInvalidation,
    );
  } finally {
...
test("compact summary reuses compact-thinking's sweep animation", () => {
  ...
  expect(first.replace(/<[^>]+>/g, "")).toBe("Thinking...");
  expect(second.replace(/<[^>]+>/g, "")).toBe("Thinking...");
});
...
test("compact thinking patches the runtime component with ccstyle config", () => {
  ...
  try {
    ...
    expect(AssistantMessageComponent.prototype.updateContent).toBe(original);
    emit("session_start", {}, tuiCtx);
    expect(AssistantMessageComponent.prototype.updateContent).not.toBe(
      original,
    );
    expect(
...
test("reload keeps the replacement compact-thinking prototype patch", () => {
  ...
  try {
    ...
    expect(replacementPatch).not.toBe(firstPatch);
    first.emit("session_shutdown", {}, tuiCtx);
    expect(AssistantMessageComponent.prototype.updateContent).toBe(
      replacementPatch,
    );
...
test("headless subagent runtime does not steal the parent thinking patch", () => {
  ...
  try {
    ...
    expect(parentPatch).not.toBe(original);
...
function thinkingMessage(timestamp: number, withAgent = true) {
  return {
    ...
    content: [
      ...
      ...(withAgent
        ? [{ type: "toolCall", name: "Agent", arguments: {} }]
        : []),
    ],
...
function themeCtx(
  sessionManager: any = { getBranch: () => [], getEntries: () => [] },
) {
  return {
    mode: "tui",
    sessionManager,
...
test("compact summary keeps the shared animation alive until the next assistant message", async () => {
  ...
  try {
    ...
    expect(controller.isMessageThinkingActive?.(message.timestamp)).toBe(false);
    const runningFrame = controller.getThinkingAnimationFrame?.() ?? 0;
    await new Promise((resolve) => setTimeout(resolve, 70));
    expect(
      (controller.getThinkingAnimationFrame?.() ?? 0) > runningFrame,
    ).toBeTruthy();

    controller.setCompactSummaryActive?.(false);
    const stoppedFrame = controller.getThinkingAnimationFrame?.() ?? 0;
    await new Promise((resolve) => setTimeout(resolve, 70));
    expect(controller.getThinkingAnimationFrame?.()).toBe(stoppedFrame);
  } finally {
    emit("session_shutdown", {}, uiCtx);
  }
});
...
test("Agent tool execution keeps the thinking animation until the next boundary", async () => {
  ...
  try {
    ...
    await new Promise((resolve) => setTimeout(resolve, 60));
    expect(controller.isMessageThinkingActive?.(ts)).toBe(true);
    expect(
    ...
    expect(controller.isMessageThinkingActive?.(ts)).toBe(false);
    expect(
      (controller.getMessageThinkingDurationMs?.(ts) ?? 0) > 0,
...
test("thinking preview counts wrapped hidden lines and does not restyle from cache", () => {
  ...
  try {
    ...
    const firstPlain = firstLines
      .map((line: string) => line.trim())
      .filter(Boolean);
    const hint = firstPlain.find(
      (line: string) => /Thought/.test(line) && /more lines/.test(line),
    );
    expect(
      hint,
    ...
    expect(
      !firstPlain.some(
        (line: string) =>
          new RegExp(`^<${bodyToken}>x+`).test(line) && /more line/.test(line),
      ),
    ...
    expect(
      firstPlain.filter((line: string) =>
        new RegExp(`^<${bodyToken}>x+`).test(line),
      ).length <= previewLines,
    ...
    expect(
      secondPlain.some((line: string) =>
        /\[dim\] • \(\d+ more lines/.test(line),
      ),
    ...
    expect(
      secondPlain.every(
        (line: string) =>
          !line.includes("<thinkingText>") && !line.includes("<dim>"),
      ),
...
test("previewLines 0 still offers click to show more and expands the body", () => {
  ...
  try {
    ...
    expect(expanded.some((line) => line.includes("line-0"))).toBeTruthy();
    expect(expanded.some((line) => line.includes("line-7"))).toBeTruthy();
  } finally {
    emit("session_shutdown", {}, ctx);
    if (previousDir === undefined) delete process.env.PI_CODING_AGENT_DIR;
    else process.env.PI_CODING_AGENT_DIR = previousDir;
...
function thinkingBlockOf(component: any): ThinkingPreviewBlock {
  ...
  expect(
    block instanceof ThinkingPreviewBlock,
    "assistant mounts a thinking preview block",
  ).toBeTruthy();
  return block;
}
...
test("thinking preview expands the full body and keeps that state across updateContent", () => {
  const dir = mkdtempSync(
    join(tmpdir(), "pi-compact-thinking-preview-expand-"),
  );
  const previousDir = process.env.PI_CODING_AGENT_DIR;
  process.env.PI_CODING_AGENT_DIR = dir;
  const { emit, pi } = runtime();
  const ctx = themeCtx();
  (ctx.ui.theme as any).bg = (_slot: string, text: string) =>
    `<bg>${text}</bg>`;
  const previewLines = 3;
  ...
  try {
    ...
    const collapsed = renderText(component);
    expect(
      collapsed.some((line) => /more lines.*click to show more/.test(line)),
    ).toBeTruthy();
    expect(
    ...
    const expanded = renderText(component);
    expect(!expanded.some((line) => line.includes("more line"))).toBeTruthy();
    expect(expanded.some((line) => line.includes("line-0"))).toBeTruthy();
    expect(expanded.some((line) => line.includes("line-19"))).toBeTruthy();
    expect(
    ...
    component.updateContent(msg);
    expect(thinkingBlockOf(component).expanded).toBe(true);
    expect(
      renderText(component).some((line) => line.includes("line-0")),
    ).toBeTruthy();

    thinkingBlockOf(component).setExpanded(false);
    const recollapsed = renderText(component);
    expect(
      recollapsed.some((line) => /more lines.*click to show more/.test(line)),
    ).toBeTruthy();
    expect(recollapsed.filter((line) => /^line-\d+$/.test(line)).length).toBe(
      previewLines,
    );
  } finally {
    emit("session_shutdown", {}, ctx);
