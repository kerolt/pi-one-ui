...
function highTierStart(frame: string): number | undefined {
  ...
  return (
    visibleWidth(stripTerminalSequences(frame.slice(0, marker.index))) -
    rowOffset
  );
}
...
function phaseSignature(frame: string): [string, number | undefined] {
  return [
    stripTerminalSequences(frame).split(" ", 1)[0] ?? "",
    highTierStart(frame),
  ];
}
...
describe("working-line token formatting", () => {
  ...
  it("reserves compact maximum-safe tokens within the complete 80-cell row", () => {
    ...
    const composed = composeWorkingLineRow(
...
describe("working-line presets and frame generation", () => {
  ...
  it("defines the exact fixed-width spinner frames without preset-owned cadences", () => {
    ...
    for (const [id, preset] of Object.entries(WORKING_LINE_SPINNERS)) {
      ...
      expect(
        workingLineSpinnerWidth(id as keyof typeof WORKING_LINE_SPINNERS),
      ).toBe(id === "pulse" ? 3 : 1);
    }
  });

  it("rejects zero-width and inconsistent spinner definitions defensively", () => {
    const mutable = WORKING_LINE_SPINNERS as unknown as {
      pulse: { frames: string[] };
    };
    const original = mutable.pulse.frames;
    try {
      mutable.pulse.frames = ["", ""];
      expect(() => workingLineSpinnerWidth("pulse")).toThrow(
  ...
  it("builds standalone spinner frames for preset validation", () => {
    ...
    expect(generated.frames.some((frame) => frame.includes("Working"))).toBe(
      false,
    );
  });
  ...
  it("renders exact owned Pulse frames in preset order", () => {
    ...
    expect(strippedFrames(generated.frames)).toEqual(
      expected.map((frame) => `${frame} Ready`),
    );
  ...
  it.each([
    ...
    "keeps the %s %s spinner fixed while text tiers move",
    (textAnimation, spinner) => {
      const current = config();
  ...
  it.each([30, 100, 180, 1000] as const)(
    ...
    (intervalMs) => {
      ...
      expect(owned.scheduler.effectiveSpinnerIntervalMs).toBeCloseTo(
        intervalMs,
        8,
      );
    },
  ...
  it.each([
    ...
    "generates deterministic %s full-row frames",
    (textAnimation, count, intervalMs) => {
      const current = config();
      current.components.workingLine.textAnimation = textAnimation;
      ...
      expect(result.frames.every((frame) => frame.endsWith("\x1b[0m"))).toBe(
        true,
      );
  ...
  it.each([
    ...
    "schedules exact independent %s/%s ms cadences",
    (spinnerMs, textMs) => {
      const current = config();
      ...
      expect(generated.intervalMs).toBe(
        Math.max(30, gcdForTest(spinnerMs, textMs)),
      );
    },
  );
  ...
  it("regenerates repeated worst-case streaming rows within a bounded budget", () => {
    ...
    for (let output = 1; output <= 20; output++) {
      ...
      expect(generated.frames.length).toBeLessThanOrEqual(
        MAX_WORKING_LINE_FRAMES,
      );
      expect(
        generated.frames.reduce((sum, frame) => sum + frame.length, 0),
      ).toBeLessThanOrEqual(MAX_WORKING_LINE_FRAME_CODE_UNITS);
    }
  ...
  it("permits one Classic text reset per bounded fallback array cycle", () => {
    ...
    const textCycle = 15;
    expect(
      new Set(generated.frameStates.map(({ textTick }) => textTick)).size,
    ).toBe(textCycle);
    expect(generated.scheduler.textAdvances % textCycle).toBe(3);
    expect(generated.scheduler.effectiveTextIntervalMs).toBeCloseTo(
      29910 / 33,
      8,
    );
  ...
  it.each([
    ...
    "uses configured cadence with the %s static frames",
    (spinner, frameCount) => {
      const current = config();
      current.components.workingLine.spinner = spinner;
  ...
  it.each([
    ...
    (animation, _counts) => {
      ...
      >) {
        ...
        expect(
          visibleWidth(stripTerminalSequences(generated.frames[0] ?? "")),
        ).toBe(MAX_WORKING_LINE_FRAME_CELLS);
  ...
  it.each(["classic", "kitt"] as const)(
    ...
    (animation) => {
      ...
      expect(generated.frames.some((frame) => frame.includes("\x1b[1m"))).toBe(
        true,
      );
    },
  );
  ...
  it.each([
    ...
    [
      ...
      "\x1b[38;5;202m\x1b[4m\x1b[3m\x1b[1m",
    ],
  ] as const)(
  ...
  it("reverses full-row KITT motion without duplicating the wording endpoint", () => {
    ...
    const brightHead = (frame: string) =>
      frame.match(/\x1b\[96m\x1b\[1m([a-h])/)?.[1];
    ...
    expect(movingHeads.slice(0, 9)).toEqual([
      "a",
      "b",
      "c",
      "d",
  ...
  it.each(["界", "😀"])(
    "gives every occupied KITT head cell the high tier for %s",
    (message) => {
      const current = config();
      current.components.workingLine.textAnimation = "kitt";
      const result = buildWorkingLineFrames(
      ...
      expect(
        result.frames.some((frame) =>
          frame.includes(`\x1b[96m\x1b[1m${message}`),
        ),
      ).toBe(true);
  ...
  it.each(["star-bloom", "pulse"] as const)(
    ...
    (spinner) => {
      ...
      expect(visibleWidth(stripTerminalSequences(frame))).toBe(
        MAX_WORKING_LINE_FRAME_CELLS,
      );
      ...
      try {
        ...
        expect(visibleWidth(rendered[1] ?? "")).toBe(
          MAX_WORKING_LINE_ROW_CELLS,
        );
      } finally {
        loader.stop();
  ...
  it("compensates for the real Loader resetting every installed indicator to frame zero", () => {
    ...
    try {
      ...
      expect(stripTerminalSequences(loader.render(20)[1] ?? "")).toContain(
        "two",
      );
  ...
  it("keeps preview mode and configured segment presence stable", () => {
    const current = config();
    current.components.workingLine.messages = {
      custom: true,
      values: ["Configured"],
    };
    ...
    expect(
      fallback.every((frame) => frame.includes(" Working… · read · ↑1.2k ↓56")),
    ).toBe(true);
  });
});
...
describe("working-line phase mapping", () => {
  it.each([
    ...
    (animation, tick) => {
      const included = remapWorkingLineTextTick(
        animation,
        10,
        tick,
        animation,
      ...
      const restored = remapWorkingLineTextTick(
        animation,
        12,
        included,
        animation,
...
describe("working-line message normalization and selection", () => {
  ...
  it("removes C1 sequences and embedded invisible controls while preserving grapheme joiners", () => {
    ...
    expect(
      normalizeWorkingLineMessage(
        "pay\u034fpal word\u2060joiner bidi\u200emark",
      ),
    ).toBe("paypal wordjoiner bidimark");
    expect(normalizeWorkingLineMessage("👩‍💻 ❤️ 👨‍👩‍👧‍👦 क्‍ष")).toBe(
      "👩‍💻 ❤️ 👨‍👩‍👧‍👦 क्‍ष",
    );
  });
  ...
  it("bounds raw normalization and invalid-entry examination before output caps", () => {
    ...
    expect(
      normalizeWorkingLineMessage(
        `${"\u200b".repeat(MAX_WORKING_LINE_RAW_CODE_UNITS - 1)}👩‍💻`,
      ),
    ).toBe("");
    const combiningGrapheme = `x${"\u0338".repeat(20)}`;
    const combiningOutput = normalizeWorkingLineMessage(
      combiningGrapheme.repeat(43),
    );
    expect(combiningOutput).toBe(
      combiningGrapheme.repeat(
        Math.floor(
          MAX_WORKING_LINE_NORMALIZED_CODE_UNITS / combiningGrapheme.length,
        ),
      ),
    );
    expect(combiningOutput.length).toBeLessThanOrEqual(
      MAX_WORKING_LINE_NORMALIZED_CODE_UNITS,
    );
    ...
    expect(
      fallback.frames.reduce((sum, frame) => sum + frame.length, 0),
    ).toBeLessThanOrEqual(MAX_WORKING_LINE_FRAME_CODE_UNITS);
    const values = [
      ...Array.from({ length: 31 }, (_, index) => `valid-${index}`),
      ...Array.from(
        { length: MAX_WORKING_LINE_ENTRIES_EXAMINED - 32 },
        () => "\u200b",
      ),
      "last-valid",
      "not-examined",
  ...
  it("supports custom toggle, fallback, and uniform endpoint-safe selection", () => {
    ...
    expect(effectiveWorkingLineMessages(current)).toEqual(
      BUILT_IN_WORKING_LINE_MESSAGES,
    );
...
describe("working-line row composition", () => {
  it("sanitizes tools and applies bounded wording, tool, elapsed, token priority", () => {
    ...
    expect(normalizeWorkingLineToolLabel("a".repeat(30))).toBe(
      `${"a".repeat(17)}…`,
    );
    ...
    expect(2 + visibleWidth(all.row) + 1 + 2).toBeLessThanOrEqual(
      MAX_WORKING_LINE_ROW_CELLS,
    );
    ...
    expect(constrained.row).toBe(
      `${"x".repeat(43)} · ↑9007199255M ↓9007199255M`,
    );
    expect(
      workingLineSpinnerWidth("pulse") + 1 + visibleWidth(constrained.row),
    ).toBeLessThan(MAX_WORKING_LINE_FRAME_CELLS);
  ...
  it("reserves exact Tokens, then allocates Message, Elapsed, and truncated Tool within 80 cells", () => {
    ...
    expect(composed.row).toBe(
      `${"x".repeat(43)} · parallel-to… · 1m02s · ↑12 ↓3`,
    );
    expect(composed.row.indexOf("1m02s")).toBeLessThan(
      composed.row.indexOf("↑12 ↓3"),
    );
    expect(composed.row).not.toContain("parallel-tool-label");
    expect(
      workingLineSpinnerWidth(component.spinner) +
        1 +
        visibleWidth(composed.row) +
        3,
    ).toBe(MAX_WORKING_LINE_ROW_CELLS);
  });
  ...
  it("honors independent segment toggles for custom and fallback wording", () => {
    ...
    expect(composeWorkingLineRow(component, "Configured", runtime).row).toBe(
      "Configured · 1s",
    );
    component.messages.custom = false;
    expect(composeWorkingLineRow(component, "Working…", runtime).row).toBe(
      "Working… · 1s",
    );
  });
});
...
describe("working-line thought segment", () => {
  it("formats active zero and floors active/inactive duration consistently", () => {
    expect(formatWorkingLineThought({ durationMs: 0, active: true })).toBe(
      "thinking 0s",
    );
    expect(formatWorkingLineThought({ durationMs: 10_999, active: true })).toBe(
      "thinking 10s",
    );
    expect(
      formatWorkingLineThought({ durationMs: 10_999, active: false }),
    ).toBe("thought for 10s");
    expect(
      formatWorkingLineThought({ durationMs: 0, active: false }),
    ).toBeUndefined();
  });
...
describe("working-line runtime ownership", () => {
  ...
  it("fails open when either public capability is missing", () => {
    ...
    for (const ui of [
      ...
      const result = harness.controller.startSession({
        hasUI: true,
        mode: "tui",
        ui,
      });
  ...
  it("owns and styles the full fallback row with no RNG when custom messages are off", () => {
    ...
    const indicator = harness.calls.at(-1)?.[1] as
      | { frames: string[] }
      | undefined;
    expect(
  ...
  it("reconciles live message edits deterministically without rerolling the turn", () => {
    ...
    harness.current.components.workingLine.messages = {
      custom: true,
      values: ["A", "B"],
    };
    harness.controller.reconcile(harness.ctx);
  ...
  it("reinstalls full rows only for visible token/tool changes and tracks parallel tools", () => {
    ...
    const text = () =>
      stripTerminalSequences(
        (harness.calls.at(-1)?.[1] as { frames?: string[] } | undefined)
          ?.frames?.[0] ?? "",
      );
  ...
  it.each([
    ...
    (animation, spinner, firstChangeAt) => {
      ...
      const activationIndex =
        Math.floor(firstChangeAt / activation.intervalMs) %
        activation.frames.length;
      harness.controller.updateTokens({ input: 12, output: 3 }, harness.ctx);
      const withTokens = harness.calls.at(-1)?.[1] as { frames: string[] };
      const beforeSignature = phaseSignature(
        activation.frames[activationIndex] ?? "",
      );
      const afterSignature = phaseSignature(withTokens.frames[0] ?? "");
      expect(afterSignature[0]).toBe(beforeSignature[0]);
      if (beforeSignature[1] !== undefined)
        expect(afterSignature[1]).toBe(beforeSignature[1]);

      now += 100;
      harness.controller.startTool("one", "read", harness.ctx);
      const withTool = harness.calls.at(-1)?.[1] as {
        frames: string[];
        intervalMs: number;
      };
      const carriedRemainder = firstChangeAt % withTool.intervalMs;
      const withTokensIndex = Math.floor(
        (carriedRemainder + 100) / withTool.intervalMs,
      );
  ...
  it.each([
    ...
    [
      "width",
      (current: PolishedTuiConfig): void => {
        current.components.workingLine.messages.values = [
          "A much wider stable message",
        ];
      },
    ],
    [
    ...
    "carries fractional phase across a live %s change",
    (_kind, change) => {
      let now = 0;
      const harness = runtime(
        true,
      ...
      harness.current.components.workingLine.segments.elapsed = false;
      harness.controller.startSession(harness.ctx);
      const before = harness.calls.at(-1)?.[1] as {
        frames: string[];
        intervalMs: number;
      };
      ...
      harness.controller.reconcile(harness.ctx);
      const after = harness.calls.at(-1)?.[1] as {
        frames: string[];
        intervalMs: number;
      };
  ...
  it("matches uninterrupted exact independent spinner/text schedules during rapid rebuilds", () => {
    ...
    harness.current.components.workingLine.segments.elapsed = false;
    harness.controller.startSession(harness.ctx);
    const uninterrupted = harness.calls.at(-1)?.[1] as {
      frames: string[];
      intervalMs: number;
    };
    ...
    for (now = 30; now <= 600; now += 30) {
      ...
      expect(phaseSignature(rebuilt.frames[0] ?? "")).toEqual(
        phaseSignature(
          uninterrupted.frames[(now / 30) % uninterrupted.frames.length] ?? "",
        ),
      );
    }
  });
  ...
  it("bounds visible Loader reset jitter below one replacement interval", () => {
    ...
    try {
      ...
      harness.current.components.workingLine.textAnimation = "disabled";
      harness.current.components.workingLine.segments.elapsed = false;
      const recordIndicator = harness.ctx.ui.setWorkingIndicator.bind(
        harness.ctx.ui,
      );
      ...
      now = 90;
      harness.controller.updateTokens({ input: 1, output: 1 }, harness.ctx);
      const replacement = harness.calls.at(-1)?.[1] as {
        frames: string[];
        intervalMs: number;
      };
      const logicalBoundaryAt = 100;
      const visibleReplacementBoundaryAt = now + replacement.intervalMs;
      expect(visibleReplacementBoundaryAt - logicalBoundaryAt).toBeLessThan(
        replacement.intervalMs,
      );
  ...
  it("recovers the last successful epoch when reconcile mutates then throws and retries on tokens", () => {
    ...
    expect(harness.calls.filter(([name]) => name === "indicator")).toHaveLength(
      3,
    );
    ...
    const failed = failedAttempt as unknown as {
      frames: string[];
      intervalMs: number;
    };
  ...
  it.each(["classic", "kitt"] as const)(
    ...
    (animation) => {
      ...
      try {
        ...
        const atNine = harness.calls.at(-1)?.[1] as {
          frames: string[];
          intervalMs: number;
        };
        expect(stripTerminalSequences(atNine.frames[0] ?? "")).toContain(
          " · 9s",
        );
        vi.advanceTimersByTime(1000);
        const atTen = harness.calls.at(-1)?.[1] as { frames: string[] };
        expect(stripTerminalSequences(atTen.frames[0] ?? "")).toContain(
          " · 10s",
        );
        const sampledIndex =
          Math.floor(1000 / atNine.intervalMs) % atNine.frames.length;
  ...
  it("writes elapsed only when its one-second label changes", () => {
    ...
    expect(
      stripTerminalSequences(
        (harness.calls[0]?.[1] as { frames?: string[] } | undefined)
          ?.frames?.[0] ?? "",
      ),
    ).toContain(" · 1s");
    vi.advanceTimersByTime(1000);
  ...
  it("transitions thought-only clock subscription strictly with active thought", () => {
    ...
    try {
      ...
      harness.controller.updateMetrics(
        undefined,
        { durationMs: 0, active: true },
        harness.ctx,
      );
      expect(vi.getTimerCount()).toBe(1);
      harness.controller.updateMetrics(
        undefined,
        { durationMs: 1, active: true },
        harness.ctx,
      );
      harness.controller.startTurn(harness.ctx);
      expect(vi.getTimerCount()).toBe(1);
    ...
    const ctx = {
      ...
      ui: {
        ...
        setWorkingIndicator(value?: unknown) {
          ...
          if (value !== undefined)
            retriedFrames = (value as { frames: string[] }).frames;
        },
      },
    };
  ...
  it("live-enables and resets indicator then message exactly once on disable or cleanup", () => {
    ...
    harness.controller.dispose(harness.ctx);
    expect(
      harness.calls.map(([name, value]) => [
        name,
        value === undefined ? "reset" : "set",
      ]),
    ).toEqual([
      ["message", "set"],
      ["indicator", "set"],
