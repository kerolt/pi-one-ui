...
describe("live interaction token display", () => {
  ...
  it("counts cumulative Unicode code points and ignores unchanged or stale snapshots", () => {
    ...
    tracker.agentStart();
    tracker.turnStart();
    const first = tracker.messageUpdate(
      assistant(0, 0),
      delta("text_delta", 0, "💡ab", "💡ab"),
    );
    expect(first.displayTokens).toEqual({
      input: 0,
      output: 1,
      outputApproximate: true,
    });
    ...
    expect(
      tracker.messageUpdate(assistant(0, 0), delta("text_delta", 0, "x", "💡a"))
        .usageChanged,
    ).toBe(false);
  });
  ...
  it("counts split surrogate pairs once in raw fallback and saturates estimates", () => {
    ...
    expect(
      huge.messageUpdate(
        assistant(0, Number.MAX_SAFE_INTEGER),
        delta("text_delta", 0, "more"),
      ).displayTokens,
    ).toEqual({
      input: 0,
  ...
  it("anchors estimates to advancing provider output and final corrects downward", () => {
    ...
    expect(
      tracker.messageUpdate(
        assistant(10, 2),
        delta("text_delta", 0, "ijkl", "abcdefghijkl"),
      ).displayTokens,
    ...
    expect(tracker.messageEnd(assistant(10, 2))).toEqual(
      accepted({ input: 10, output: 2 }),
    );
  ...
  it("handles sparse cumulative thinking indexes and divergent snapshots conservatively", () => {
    ...
    expect(
      tracker.messageUpdate(
        assistant(0, 0),
        delta("thinking_delta", 7, "abcd", "abcd"),
      ).displayTokens,
    ).toEqual({ input: 0, output: 1, outputApproximate: true });
    expect(
      tracker.messageUpdate(
        assistant(0, 0),
        delta("thinking_delta", 7, "efgh", "abcdefgh"),
      ).displayTokens,
    ).toEqual({ input: 0, output: 2, outputApproximate: true });
    expect(
      tracker.messageUpdate(
        assistant(0, 0),
        delta("thinking_delta", 7, "zzzz", "abcdzzzz"),
      ).usageChanged,
    ).toBe(false);
  ...
  it("validates malformed event boundaries before any mutation", () => {
    for (const event of [
      ...
      tracker.turnStart(0);
      expect(() =>
        tracker.messageUpdate(assistant(-1, 1), event as never, 10),
      ).not.toThrow();
      expect(tracker.currentTokens()).toEqual({ input: 0, output: 0 });
      expect(tracker.currentThought(20)).toEqual({
        durationMs: 0,
        active: false,
      });
  ...
  it("releases bounded estimator state on closure", () => {
    ...
    expect(tracker.messageEnd(assistant(2, 1))).toEqual(
      accepted({ input: 2, output: 1 }),
    );
    expect(tracker.diagnostics()).toMatchObject({
      activeResponses: 0,
      contentBlocks: 0,
  ...
  it("processes progressive cumulative snapshots incrementally with bounded retention", () => {
    ...
    for (let index = 0; index < 20_000; index++) {
      ...
      tracker.messageUpdate(
        assistant(0, 0),
        delta("text_delta", 0, "x", cumulative),
      );
    }
  ...
  it("bounds block, delta, snapshot, generated, and thought estimator state without losing final usage", () => {
    ...
    expect(() =>
      tracker.messageUpdate(
        assistant(0, 0),
        delta("text_delta", 0, "x".repeat(65_537)),
      ),
    ).not.toThrow();
    expect(() =>
      tracker.messageUpdate(
        assistant(0, 0),
        delta("text_delta", 0, "x", "x".repeat(1_048_577)),
      ),
    ).not.toThrow();
    ...
    expect(tracker.messageEnd(assistant(11, 7), 300)).toEqual(
      accepted({ input: 11, output: 7 }),
    );
  });
  ...
  it("counts every raw tool-call chunk independently and exact final usage reconciles it", () => {
    ...
    tracker.turnStart(0);
    expect(
      tracker.messageUpdate(assistant(0, 0), toolDelta(0, "abcdefgh"), 10),
    ).toMatchObject({
    ...
    expect(
      tracker.messageUpdate(assistant(0, 0), toolDelta(0, "abcdefgh"), 20)
        .displayTokens,
    ).toEqual({ input: 0, output: 4, outputApproximate: true });
    expect(tracker.currentThought(30)).toEqual({
      durationMs: 0,
      active: false,
    });
    expect(tracker.diagnostics()).toMatchObject({
    ...
    expect(tracker.messageEnd(assistant(7, 1), 40)).toEqual(
      accepted({ input: 7, output: 1 }),
    );
  });
  ...
  it("ceilings mixed text, thinking, and tool-call output together", () => {
    ...
    tracker.messageUpdate(
      assistant(0, 0),
      delta("thinking_delta", 1, "def", "def"),
    );
    expect(
      tracker.messageUpdate(assistant(0, 0), toolDelta(2, "ghi")).displayTokens,
    ).toEqual({
      input: 0,
      output: 3,
      outputApproximate: true,
  ...
  it("rejects malformed tool deltas atomically and bounds oversized tool-call state", () => {
    for (const event of [
      ...
      tracker.turnStart();
      expect(
        tracker.messageUpdate(assistant(0, 0), event as never).usageChanged,
      ).toBe(false);
      expect(tracker.diagnostics()).toMatchObject({
        contentBlocks: 0,
    ...
    oversized.turnStart();
    expect(
      oversized.messageUpdate(assistant(0, 0), toolDelta(0, "x".repeat(65_537)))
        .usageChanged,
    ).toBe(true);
    ...
    expect(oversized.messageEnd(assistant(11, 7))).toEqual(
      accepted({ input: 11, output: 7 }),
    );
  });
  ...
  it("caps tool-call blocks and preserves exact final reconciliation", () => {
    ...
    expect(tracker.messageEnd(assistant(9, 4))).toEqual(
      accepted({ input: 9, output: 4 }),
    );
    expect(tracker.diagnostics()).toMatchObject({
      contentBlocks: 0,
      retainedContentUnits: 0,
...
describe("interaction metrics tracker", () => {
  it("promotes a response-less stream to a normalized ID and ignores its duplicate final", () => {
    ...
    expect(
      tracker.messageUpdate(
        assistant(100, 4, { timestamp: 1, provider: "first", model: "first" }),
      ).displayTokens,
    ...
    expect(
      tracker.messageEnd(
        assistant(150, 9, { responseId: "response-a", timestamp: 3 }),
      ),
    ).toEqual(accepted({ input: 150, output: 9 }));
    expect(
      tracker.messageEnd(
        assistant(999, 99, { responseId: " response-a ", timestamp: 4 }),
      ),
  ...
  it("keeps short IDs exact and treats overlong IDs as anonymous", () => {
    ...
    tracker.turnStart();
    expect(
      tracker.messageEnd(assistant(20, 2, { responseId: second })),
    ).toEqual(accepted({ input: 30, output: 3 }));
    tracker.turnStart();
    expect(
      tracker.messageEnd(assistant(99, 99, { responseId: first })),
    ).toEqual({
      status: "duplicate",
    });
    ...
    tracker.turnStart();
    expect(
      tracker.messageEnd(assistant(3, 4, { responseId: overlong })),
    ).toEqual(accepted({ input: 33, output: 7 }));
  ...
  it("rejects multi-megabyte response IDs before normalization or retention", () => {
    ...
    expect(
      tracker.messageUpdate(assistant(2, 1, { responseId: overlong }))
        .displayTokens,
    ).toEqual({
      input: 2,
      output: 1,
    ...
    expect(
      tracker.messageEnd(assistant(7, 3, { responseId: overlong })),
    ).toEqual(accepted({ input: 7, output: 3 }));
    expect(performance.now() - startedAt).toBeLessThan(1_000);
    expect(tracker.diagnostics().responseIds).toBe(0);
    expect(
      tracker.messageEnd(assistant(99, 99, { responseId: overlong })),
    ).toEqual({
      status: "rejected",
    });
  });
  ...
  it("counts a reused response ID independently in a later low-level run", () => {
    ...
    tracker.turnStart();
    expect(
      tracker.messageEnd(assistant(20, 2, { responseId: " reused " })),
    ).toEqual(
      accepted({
        input: 30,
        output: 3,
  ...
  it("rejects distinct finals until turn_start authorizes each slot", () => {
    ...
    tracker.agentStart();
    expect(
      tracker.messageEnd(assistant(10, 1, { responseId: "a", timestamp: 7 })),
    ).toEqual({
      status: "rejected",
    });
    tracker.turnStart();
    expect(
      tracker.messageEnd(assistant(20, 2, { responseId: "b", timestamp: 7 })),
    ).toEqual(accepted({ input: 20, output: 2 }));
  });
  ...
  it("deduplicates a response-less aborted streaming final", () => {
    ...
    tracker.turnStart();
    tracker.messageUpdate(
      assistant(4, 1, { stopReason: "aborted", timestamp: 1 }),
    );
    expect(
      tracker.messageEnd(
        assistant(6, 2, { stopReason: "aborted", timestamp: 2 }),
      ),
    ).toEqual(
      accepted({
        input: 6,
    ...
    expect(
      tracker.messageEnd(
        assistant(60, 20, { stopReason: "aborted", timestamp: 3 }),
      ),
    ).toEqual({
      status: "rejected",
    });
  ...
  it("displays cumulative usage throughout a three-response tool loop", () => {
    ...
    for (const [
      ...
      expect(
        tracker.messageUpdate(
          assistant(input - 1, output, { responseId, stopReason }),
        ).displayTokens,
      ).toEqual(expectedUpdate);
      expect(
        tracker.messageEnd(
          assistant(input, output, { responseId, stopReason }),
        ),
      ).toEqual(accepted(expectedFinal));
    }
  ...
  it("retains committed totals for a next response's initial zero snapshot", () => {
    ...
    tracker.turnStart();
    expect(
      tracker.messageUpdate(assistant(0, 0, { responseId: "second" }))
        .displayTokens,
    ).toEqual({
      input: 10,
      output: 1,
    ...
    expect(
      tracker.messageUpdate(assistant(7, 2, { responseId: "second" }))
        .displayTokens,
    ).toEqual({
      input: 17,
      output: 3,
  ...
  it("shows repeated initial zero updates once and ignores placeholder regression", () => {
    ...
    tracker.turnStart();
    expect(
      tracker.messageUpdate(assistant(0, 0, { responseId: "stream" }))
        .displayTokens,
    ).toEqual({
      input: 0,
      output: 0,
    ...
    expect(tracker.currentTokens()).toEqual({ input: 0, output: 0 });
    expect(
      tracker.messageUpdate(assistant(8, 2, { responseId: "stream" }))
        .displayTokens,
    ).toEqual({
      input: 8,
      output: 2,
  ...
  it("commits a zero-only final exactly once", () => {
    ...
    tracker.turnStart();
    expect(
      tracker.messageUpdate(assistant(0, 0, { responseId: "zero-only" }))
        .displayTokens,
    ).toEqual({ input: 0, output: 0, outputApproximate: true });
    expect(
      tracker.messageEnd(assistant(0, 0, { responseId: "zero-only" })),
    ).toEqual(
      accepted({
        input: 0,
        output: 0,
    ...
    expect(
      tracker.messageEnd(assistant(9, 3, { responseId: "zero-only" })),
    ).toEqual({
      status: "rejected",
    });
    tracker.agentEnd();
  ...
  it("rejects invalid and non-assistant updates without changing accounting", () => {
    ...
    expect(
      tracker.messageUpdate({ role: "user", usage: { input: 99, output: 99 } })
        .usageChanged,
    ).toBe(false);
    expect(tracker.currentTokens()).toEqual({ input: 8, output: 2 });
  });
  ...
  it("saturates token fields independently and settles valid data with a clamped duration", () => {
    ...
    tracker.turnStart();
    expect(
      tracker.messageEnd(assistant(-1, 4, { responseId: "invalid-negative" })),
    ).toEqual(accepted({ input: 0, output: 0 }, "no-snapshot"));
    expect(
      tracker.messageEnd(
        assistant(Number.MAX_SAFE_INTEGER + 1, 4, {
          responseId: "invalid-unsafe",
        }),
      ),
    ).toEqual({ status: "rejected" });
    tracker.turnStart();
    tracker.messageEnd(
      assistant(Number.MAX_SAFE_INTEGER - 2, 10, { responseId: "first-valid" }),
    );
    tracker.turnStart();
    expect(
      tracker.messageEnd(
        assistant(5, Number.MAX_SAFE_INTEGER - 20, {
          responseId: "second-valid",
        }),
      ),
    ).toEqual(
      accepted({
    ...
    tracker.turnStart();
    expect(
      tracker.messageEnd(assistant(0, 20, { responseId: "third-valid" })),
    ).toEqual(
      accepted({
        input: Number.MAX_SAFE_INTEGER,
        output: Number.MAX_SAFE_INTEGER,
  ...
  it("preserves a live estimate when a malformed final has no valid snapshot", () => {
    ...
    expect(
      tracker.messageEnd(assistant(-1, 1, { responseId: "invalid" })),
    ).toEqual(
      accepted({ input: 0, output: 0 }, "no-snapshot", {
        input: 0,
        output: 2,
  ...
  it("preserves an estimate above a lower provider anchor on malformed final", () => {
    ...
    expect(
      tracker.messageEnd(assistant(-1, 9, { responseId: "anchored" })),
    ).toEqual(
      accepted({ input: 10, output: 1 }, "last-snapshot", {
        input: 10,
        output: 4,
    ...
    tracker.agentEnd();
    expect(tracker.settle(true)?.summary).toMatchObject({
      input: 10,
      output: 1,
    });
  });
  ...
  it("rejects mismatched updates and finals without modifying or closing the active response", () => {
    ...
    expect(
      tracker.messageEnd(assistant(99, 9, { responseId: "b" }), 20),
    ).toEqual({
      status: "rejected",
    });
    expect(tracker.currentThought(30)).toEqual({
    ...
    expect(
      tracker.messageEnd(assistant(12, 2, { responseId: "a" }), 40),
    ).toEqual(
      accepted({
        input: 12,
        output: 2,
    ...
    expect(tracker.currentThought(50)).toEqual({
      durationMs: 40,
      active: false,
    });
  });
  ...
  it("does not let a mismatched malformed final close or commit the active response", () => {
    ...
    expect(
      tracker.messageEnd(assistant(-1, 9, { responseId: "b" }), 20),
    ).toEqual({
      status: "rejected",
    });
    expect(tracker.currentTokens()).toEqual({ input: 10, output: 1 });
    ...
    expect(
      tracker.messageEnd(assistant(-1, 9, { responseId: "a" }), 40),
    ).toEqual(accepted({ input: 10, output: 1 }, "last-snapshot"));
    expect(tracker.currentTokens()).toEqual({ input: 10, output: 1 });
    expect(tracker.currentThought(50)).toEqual({
      durationMs: 40,
      active: false,
  ...
  it("does not let a stale committed ID promote a fresh response-less turn slot", () => {
    ...
    tracker.turnStart();
    expect(
      tracker.messageUpdate(assistant(99, 9, { responseId: "a" })).usageChanged,
    ).toBe(false);
    expect(tracker.messageEnd(assistant(20, 2, { responseId: "b" }))).toEqual(
      accepted({
        input: 30,
  ...
  it("keeps authorized live and exact accounting at estimator and response-ID caps", () => {
    ...
    expect(
      tracker.messageEnd(assistant(2, 3, { responseId: "exact-only" })),
    ).toEqual(accepted({ input: 130, output: 131 }));
    ...
    tracker.turnStart();
    expect(
      tracker.messageEnd(assistant(3, 4, { responseId: "overflow-id" })),
    ).toEqual(accepted({ input: 260, output: 262 }));
    tracker.turnStart();
    expect(
      tracker.messageUpdate(
    ...
    expect(
      tracker.messageEnd(assistant(99, 99, { responseId: "overflow-id" })),
    ).toEqual({
      status: "duplicate",
    });
    expect(
      tracker.messageEnd(
        assistant(5, 6, { responseId: "fresh-after-overflow" }),
      ),
    ).toEqual(accepted({ input: 265, output: 268 }));
    expect(
      tracker.messageEnd(
        assistant(99, 99, { responseId: "fresh-after-overflow" }),
      ),
    ).toEqual({
      status: "rejected",
    });
    expect(tracker.messageEnd(assistant(99, 99))).toEqual({
      status: "rejected",
    });
    expect(
      tracker.messageEnd(
        assistant(99, 99, { responseId: "overflow-unsolicited" }),
      ),
    ).toEqual({
      status: "rejected",
    });
    ...
    expect(
      tracker.messageUpdate(assistant(99, 99, { responseId: "mismatch" }))
        .usageChanged,
    ).toBe(false);
    expect(
      tracker.messageEnd(assistant(99, 99, { responseId: "mismatch" })),
    ).toEqual({
      status: "rejected",
    });
    expect(
      tracker.messageEnd(assistant(9, 9, { responseId: "authorized-at-cap" })),
    ).toEqual(accepted({ input: 275, output: 278 }));
    expect(tracker.diagnostics().responseIds).toBe(256);
    expect(
      tracker.messageEnd(
        assistant(99, 99, { responseId: "authorized-at-cap" }),
      ),
    ).toEqual({
      status: "rejected",
    });
  ...
  it("ignores late events after agent_end and accepts them only after a real next agent_start", () => {
    ...
    tracker.agentEnd(10);
    expect(
      tracker.messageUpdate(assistant(90, 9, { responseId: "late" }))
        .usageChanged,
    ).toBe(false);
    expect(
      tracker.messageEnd(assistant(90, 9, { responseId: "late" })),
    ).toEqual({
      status: "rejected",
    });
    expect(tracker.currentTokens()).toEqual({ input: 10, output: 1 });
    ...
    expect(
      tracker.messageEnd(assistant(20, 2, { responseId: "late" })),
    ).toEqual(
      accepted({
        input: 30,
        output: 3,
  ...
  it("retains a live response ID and snapshot across non-idle partitioning", () => {
    ...
    expect(
      tracker.messageEnd(
        assistant(9, 3, { responseId: "live", timestamp: 99 }),
      ),
    ).toEqual(
      accepted({
        input: 9,
...
describe("thought duration tracking", () => {
  ...
  it("retains a bounded exact lower bound after adversarial subtraction", () => {
    ...
    for (let index = 0; index < result.intervals.length; index++) {
      ...
      if (index > 0)
        expect(interval.start).toBeGreaterThan(result.intervals[index - 1].end);
    }
  });
  ...
  it("counts the union of overlapping blocks and updates open elapsed from receipt time", () => {
    ...
    tracker.turnStart();
    tracker.messageUpdate(
      assistant(0, 0, { responseId: "a" }),
      thinking("thinking_start", 0),
    );
    now = 2_000;
    tracker.messageUpdate(
      assistant(0, 0, { responseId: "a" }),
      thinking("thinking_start", 1),
    );
    now = 3_000;
    expect(tracker.currentThought()).toEqual({
      durationMs: 2_000,
      active: true,
    });
    tracker.messageUpdate(
  ...
  it("keeps an exact lower-bound union after the strict thought interval cap", () => {
    ...
    expect(tracker.currentThought(2_000)).toEqual({
      durationMs: 256,
      active: false,
    });
    ...
    expect(tracker.diagnostics().thoughtIntervals).toBe(256);
    expect(tracker.currentThought(2_001)).toEqual({
      durationMs: 256,
      active: false,
    });
    ...
    expect(tracker.diagnostics().thoughtIntervals).toBe(256);
    expect(tracker.currentThought(2_100)).toEqual({
      durationMs: 256,
      active: false,
    });
  ...
  it("partitions ended thought without double-counting a concurrent retained run", () => {
    ...
    expect(tracker.currentThought(150)).toEqual({
      durationMs: 50,
      active: true,
    });
  });
...
describe("turn summary entry", () => {
  it("validates versioned numeric data and formats exact persistent text", () => {
    ...
    expect(
      stripTerminalSequences(rendered?.render(100).join("\n") ?? "").trimEnd(),
    ).toBe(" Turn took 42s · ↑27k ↓1.4k");
  });
  ...
  it("validates v3 exactly and formats thought while omitting zero", () => {
    ...
    expect(isTurnSummaryData(data)).toBe(true);
    expect(formatTurnSummary(data)).toBe(
    ...
    expect(isTurnSummaryData({ ...data, stylePrefix: "\x1b]8;;bad\x07" })).toBe(
      false,
    );
  });
  ...
  it("accepts four safe SGR sequences but rejects a fifth sequence and injected controls", () => {
    ...
    expect(rendered).toContain(
      `${safe.stylePrefix} Turn took 0s · ↑2 ↓3\x1b[0m`,
    );
    expect(
      isTurnSummaryData({ ...safe, stylePrefix: `${safe.stylePrefix}\x1b[2m` }),
    ).toBe(false);
    expect(
      isTurnSummaryData({
        ...safe,
        stylePrefix: `${safe.stylePrefix}\x1b]8;;bad\x07`,
      }),
    ).toBe(false);
    expect(
      isTurnSummaryData({
  ...
  it.each([
    ...
    expect(
      renderTurnSummaryEntry({ data }, {}, { fg: (_color, text) => text }),
    ).toBeUndefined();
  });
