...
describe("parseGitStatusPorcelain", () => {
  ...
  it("captures branch.oid on a normal branch and reports detached=false", () => {
    ...
    expect(onBranch.commit?.oid).toBe(
...
describe("detectGitState", () => {
  ...
  it("detects MERGING / CHERRY-PICKING / REVERTING / BISECTING", () => {
    ...
    expect(
      detectGitState({ mergeHead: requirePath(paths, "MERGE_HEAD") }),
    ).toEqual({
      gitState: "MERGING",
    ...
    expect(
      detectGitState({
        cherryPickHead: requirePath(paths, "CHERRY_PICK_HEAD"),
      }),
    ).toEqual({
    ...
    expect(
      detectGitState({ revertHead: requirePath(paths, "REVERT_HEAD") }),
    ).toEqual({
      gitState: "REVERTING",
      gitStateLabel: "REVERTING",
    });
    expect(
      detectGitState({ bisectLog: requirePath(paths, "BISECT_LOG") }),
    ).toEqual({
      gitState: "BISECTING",
  ...
  describe("parseGitNumstat", () => {
    it("sums added/deleted across text files", () => {
      expect(
        parseGitNumstat(
          ["10\t5\tsrc/a.ts", "3\t0\tsrc/b.ts", "0\t7\tsrc/c.ts"].join("\n"),
        ),
    ...
    it("handles rename rows (old\tnew path)", () => {
      expect(parseGitNumstat("0\t0\told.ts\tnew.ts")).toEqual({
        added: 0,
        deleted: 0,
      });
      expect(parseGitNumstat("5\t1\told.ts\tnew.ts")).toEqual({
        added: 5,
        deleted: 1,
      });
    });
    ...
    it("ignores malformed lines", () => {
      expect(
        parseGitNumstat(
          ["garbage", "", "abc\tdef\tnotnum", "5\t2\tok.ts"].join("\n"),
        ),
