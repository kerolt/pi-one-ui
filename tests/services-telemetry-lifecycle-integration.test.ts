  ...
  return {
    ...
    SettingsManager: {
      create(...args: unknown[]) {
        ...
        if (capabilities.throwAutoCompaction)
          ...
          footer: {
            ...
            styles: {
              starship: {
                ...
                segments: {
                  ...footer.styles.starship.segments,
                  modelInfo: true,
                },
              },
            },
...
function createHarness(name: string) {
  ...
  const state = {
    model: { id: `${name}-model`, provider: "test", contextWindow: 10_000 },
  };
  ...
  const ctx = {
    ...
    modelRegistry: {
      isUsingOAuth() {
        if (capabilities.throwSubscription)
          throw new Error("unsupported OAuth lookup");
...
describe("telemetry lifecycle integration", () => {
  it("initializes, refreshes, clears unsupported values, and reconciles at agent_end", async () => {
    ...
    await expect(
      emit(handlers, "session_info_changed", harness.ctx),
  ...
  it("does not leak telemetry across shutdown and replacement session start", async () => {
    ...
    expect(rendered(firstFooter)).toMatch(
      /\(sub\).*\(auto\)|\(auto\).*\(sub\)/,
    );
    ...
    expect(rendered(replacementFooter)).not.toMatch(
      /\(sub\)|\(auto\)|R1\.2k|W300/,
    );
