...
} from "../extensions/features/compact-thinking.ts";
...
test("collapsed thinking previews memoize complete output until invalidated", () => {
  ...
  try {
    ...
    assert.strictEqual(
      preview.render(12),
      first,
      "same state reuses complete output",
    );

    assert.notStrictEqual(
      preview.render(16),
      first,
      "width changes recompute output",
    );
    ...
    assert.notStrictEqual(
      preview.render(16),
      beforeHover,
      "hover changes recompute output",
    );
...
test("compact summary keeps the shared animation alive until the next assistant message", async () => {
  ...
  try {
    ...
    assert.equal(
      controller.isMessageThinkingActive?.(message.timestamp),
      false,
    );
...
test("thinking preview expands the full body and keeps that state across updateContent", () => {
  ...
  try {
    ...
    assert.equal(
      recollapsed.filter((line) => /^line-\d+$/.test(line)).length,
      previewLines,
    );
  } finally {
    emit("session_shutdown", {}, ctx);
