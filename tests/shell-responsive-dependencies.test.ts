    ...
    saveStarshipFooterStylePatch(patch: Record<string, unknown>) {
      ...
      mocks.config = {
        ...
        ...(patch.format === undefined
          ? {}
          : { footerFormat: patch.format as string }),
        ...
        ...(patch.compactMaxLines === undefined
          ? {}
          : {
              compactFooterMaxLines: patch.compactMaxLines as
                | 1
                | 2
                | 3
                | "unlimited",
        ...
        ...(patch.segments === undefined
          ? {}
          : { footerSegments: nextStarship.segments }),
        ...(patch.gitCommit === undefined
          ? {}
          : { gitCommit: nextStarship.gitCommit }),
        ...(patch.gitMetrics === undefined
          ? {}
          : { gitMetrics: nextStarship.gitMetrics }),
        components: {
          ...mocks.config.components,
            ...
            styles: {
              starship: {
                ...
                ...(patch.compactFooterMaxLines === undefined
                  ? {}
                  : {
                      compactMaxLines: patch.compactFooterMaxLines as
                        | 1
                        | 2
                        | 3
                        | "unlimited",
...
function updateStarship(
  patch: Partial<
    PolishedTuiConfig["components"]["footer"]["styles"]["starship"]
  >,
) {
  const footer = mocks.config.components.footer;
  mocks.config = {
...
beforeEach(() => {
  ...
  mocks.readRuntimeInfo
    .mockReset()
    .mockResolvedValue({ kind: "ok", runtime: undefined });
  mocks.readPackageVersionResult
    .mockReset()
    .mockResolvedValue({ kind: "ok", result: null });
...
describe("responsive footer dependency reconciliation", () => {
  ...
  it("clears package state when its reference deactivates before a failed re-enable", async () => {
    ...
    expect(state.packageVersion).toEqual({
      ecosystem: "nodejs",
      version: "1.2.3",
    });
  ...
  it("invalidates an in-flight package read across remove and failed re-enable", async () => {
    ...
    let resolveObsoleteRead:
      | ((value: {
          kind: "ok";
          result: { ecosystem: string; version: string };
        }) => void)
      | undefined;
    mocks.readPackageVersionResult
  ...
  it("counts the enabled built-in runtime segment as an active reference", async () => {
    updateStarship({
      ...
      responsive: false,
      segments: {
        ...mocks.config.components.footer.styles.starship.segments,
        runtime: true,
      },
    });
    ...
    const ctx = createContext(async (factory) => {
      ...
      for (let attempts = 0; attempts < 8; attempts++) {
        if (
          component
            .render(120)
            .some((line) => line.includes("> Editor model label"))
        )
          break;
  ...
  it("forces Git refreshes for exact-tag and submodule probe changes", async () => {
    ...
    const ctx = createContext(async (factory) => {
      ...
      const label =
        target === "showTag" ? "Show exact-match tag" : "Ignore submodules";
      for (let attempts = 0; attempts < 12; attempts++) {
        if (component.render(140).some((line) => line.includes(`> ${label}`)))
          break;
        component.handleInput("\x1b[B");
      }
      component.handleInput(" ");
    ...
    expect(mocks.readGitStatus.mock.calls.at(-1)?.[1]).toMatchObject({
      readExactTag: false,
    });
