...
vi.mock("../extensions/services/runtime-data", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("../extensions/services/runtime-data")
    >();
  return {
    ...actual,
...
vi.mock("../extensions/services/package-data", async (importOriginal) => {
  const actual =
    await importOriginal<
      typeof import("../extensions/services/package-data")
    >();
  return {
    ...actual,
    readPackageVersionResult: async () => ({
      kind: "ok" as const,
      result: null,
    }),
  };
...
describe("live streaming context event integration", () => {
  it("coalesces registered updates, uses the newest cumulative total, and finalizes canonically", async () => {
    ...
    await emit(handlers, "message_update", harness.ctx, {
      message: assistant(1_000),
    });
    await emit(handlers, "message_update", harness.ctx, {
  ...
  it("clears live usage at wired boundaries and recovers after null compaction", async () => {
    ...
    await seedLive();
    await emit(handlers, "message_end", harness.ctx, {
      message: assistant(1_100, "aborted"),
    });
    expect(rendered(footer)).toContain("10.0%/10k");
