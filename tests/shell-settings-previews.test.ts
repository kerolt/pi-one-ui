...
function expectOnlyTrustedSgr(
  rows: string[],
  hostileText: readonly string[] = [],
): void {
  for (const row of rows) {
    ...
    for (let index = 0; index < row.length; ) {
      ...
      if (code === 0x1b) {
        ...
        expect(
          sgr,
          `unexpected terminal escape in ${JSON.stringify(row)}`,
        ).not.toBeNull();
...
describe("settings previews", () => {
  it.each(widths)("bounds deterministic Editor output at width %i", (width) => {
    ...
    if (first.length > 0)
      expect(visibleWidth(first.at(-1) ?? "")).toBeGreaterThan(0);
  ...
  it.each(widths)(
    "bounds deterministic User-message output at width %i",
    (width) => {
      const current = config();
      const first = renderUserMessageSettingsPreview(current, theme(), width);
      expect(first).toEqual(
        renderUserMessageSettingsPreview(current, theme(), width),
      );
      expect(first.length).toBeLessThanOrEqual(SETTINGS_PREVIEW_MAX_ROWS);
      if (width <= 0) expect(first).toEqual([]);
      if (first.length > 0)
  ...
  it("renders the defining structure of all four User-message styles without status cues", () => {
    const outputs = Object.fromEntries(
      (
        ...
        return [
          style,
          plain(renderUserMessageSettingsPreview(current, theme(), 72)),
        ];
      }),
    ...
    const framedRows = plain(
      renderUserMessageSettingsPreview(framed, theme(), 72),
    ).split("\n");
  ...
  it("re-renders from the current theme without cached preview rows", () => {
    ...
    const firstMessage = renderUserMessageSettingsPreview(
      current,
      theme(0),
      72,
    );
    const secondMessage = renderUserMessageSettingsPreview(
      current,
      theme(37),
      72,
    );
