...
import {
  ...
  readPackageVersion,
} from "../extensions/shell/package-version";
...
function makeProject(files: Record<string, string>): string {
  ...
  for (const [name, content] of Object.entries(files)) {
    ...
    if (name.includes("/")) {
      mkdirSync(join(cwd, name.slice(0, name.indexOf("/"))), {
        recursive: true,
      });
    }
...
describe("readPackageVersion", () => {
  ...
  it("rejects terminal controls in a JSON-decoded package version", () => {
    const cwd = makeProject({
      "package.json": JSON.stringify({
        version: "1.2.3\x1b]52;c;c2VjcmV0\x07",
      }),
    });
    expect(readPackageVersion(cwd)).toBeNull();
  });
  ...
  it("parses deno.json and deno.jsonc", () => {
    const denoJson = makeProject({
      "deno.json": JSON.stringify({ version: "0.1.0" }),
    });
  ...
  it("parses Gradle gradle.properties", () => {
    const cwd = makeProject({
      "gradle.properties": "version=4.5.6\ngroup=com.example\n",
    });
...
describe("cleanVersion", () => {
  ...
  it.each(["", "   ", "not a version", "1.2.3 with spaces", "{1.2.3}"])(
    "rejects %j",
    (input) => {
