...
describe("sanitizeUserMessageSourceText", () => {
  it.each(safeTerminators)(
    "strips structurally valid 7-bit OSC 8 controls (%j)",
    (end) => {
      const open = `\x1b]8;id=docs;https://example.test/a${end}`;
      const close = `\x1b]8;;${end}`;
      expect(
        sanitizeUserMessageSourceText(`before ${open}link${close} after`),
      ).toBe("before link after");
    },
  );
  ...
  it.each([
    [
      "unmatched open",
      "\x1b]8;;https://example.test\x07before link after",
      "before link after",
    ],
  ...
  it.each(["0;title", "2;title", "52;c;c2VjcmV0", "133;A", "9;text", "hello"])(
    ...
    (payload) => {
      expect(
        sanitizeUserMessageSourceText(`before\x1b]${payload}\x07after`),
      ).toBe("beforeafter");
      expect(
        sanitizeUserMessageSourceText(`before\x9d${payload}\x9cafter`),
      ).toBe("beforeafter");
    },
  );
  ...
  it.each([
    ...
    expect(sanitizeUserMessageSourceText(`left\x1b]${payload}\x07right`)).toBe(
      "leftright",
    );
  });

  it("neutralizes unterminated and nested OSC without consuming later visible text", () => {
    expect(sanitizeUserMessageSourceText("left\x1b]133;A;visible")).toBe(
      "leftvisible",
    );
    expect(
      sanitizeUserMessageSourceText(
        "left\x1b]2;title\x1b]8;;https://safe.test\x07right",
      ),
  ...
  it("strips many raw links in one linear source pass", () => {
    ...
    const source = Array.from(
      { length: 20_000 },
      (_, index) => `${open}${index}${close}`,
    ).join(" ");
    const expected = Array.from({ length: 20_000 }, (_, index) =>
      String(index),
    ).join(" ");
    expect(sanitizeUserMessageSourceText(source)).toBe(expected);
  });
...
describe("sanitizeRenderedUserMessageText", () => {
  ...
  it("balances predecessor hyperlinks across rendered rows", () => {
    ...
    ).toEqual([`${open}first`, `second${close} ${sgr}`]);
    expect(
      sanitizeRenderedUserMessageLines([`${open}first`, `second ${sgr}`]),
    ).toEqual(["first", `second ${sgr}`]);
    expect(
      sanitizeRenderedUserMessageLines(["first", `second\x1b]8;;\x07 ${sgr}`]),
    ).toEqual(["first", `second ${sgr}`]);
  });
});
...
describe("isValidUserMessageOsc8Payload", () => {
  it("accepts parameter entries and custom URI schemes", () => {
    expect(isValidUserMessageOsc8Payload("8;id=x:foo=bar;custom:target")).toBe(
      true,
    );
  });
