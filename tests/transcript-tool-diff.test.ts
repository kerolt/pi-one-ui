...
import { shouldRenderRichDiff } from "../extensions/transcript/renderer/index.ts";
import { config } from "../extensions/app/config/renderer.ts";
import { installDefaultMode } from "../extensions/transcript/renderer/default-mode.ts";
import {
  renderEditDiffResult,
  renderWriteDiffResult,
} from "../extensions/transcript/renderer/tool/diff/diff-renderer.ts";
...
} from "../extensions/transcript/renderer/tool/diff/index.ts";
import {
  executeWriteWithMetadata,
  MAX_COMPARABLE_WRITE_BYTES,
  MAX_WRITE_METADATA_ENTRIES,
} from "../extensions/transcript/renderer/tool/diff/write-execution.ts";
...
test("diff indicator mode live-updates on the same component via config getter", () => {
  ...
  assert.match(
    classicText,
    /\+.*added line/,
    "classic mode uses +/- content markers",
  );
  assert.doesNotMatch(
    classicText,
    /• \d+ hunks? • \d+ files?/,
  ...
  assert.notEqual(
    barsText,
    classicText,
    "cache must miss when indicator mode changes",
  );
...
test("write create and overwrite render distinct rich diffs", () => {
  ...
  assert.doesNotMatch(
    overwriteCollapsed,
    /\bold\b/,
    "collapsed overwrite has no body",
  );
...
test("write execution captures the 512000-byte boundary and degrades above it", async () => {
  ...
  try {
    ...
    assert.equal(
      store.get("boundary")?.previousContent?.length,
      MAX_COMPARABLE_WRITE_BYTES,
    );
...
test("write collapsed preview uses writeDiffCollapsedLines independently of edit", () => {
  ...
  assert.doesNotMatch(
    editText,
    /\+40 -0/,
    "edit must not use write stats-only collapse",
  );
});
