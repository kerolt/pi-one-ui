...
vi.mock("../extensions/app/config/shell", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../extensions/app/config/shell")>();
  return {
    ...actual,
    ensureConfigExists() {},
    loadConfig: () => {
      ...
      config.components.workingLine.spinnerIntervalMs =
        runtime.spinnerIntervalMs;
...
import zentui from "../extensions/app/runtime/legacy-shell-adapter";
...
function loaderPhase(rendered: string): [string, number | undefined] {
  ...
  if (separatorIndex < 0)
    throw new Error("Expected a spinner separator in the working row");
  const spinner = plain.slice(0, separatorIndex);
  const textOffset =
    visibleWidth(stripped) -
    visibleWidth(plain) +
    visibleWidth(plain.slice(0, separatorIndex + 1));
  const marker = [...rendered.matchAll(/\x1b\[96m\x1b\[1m/g)].find((match) => {
    if (match.index === undefined) return false;
    return (
      visibleWidth(stripTerminalSequences(rendered.slice(0, match.index))) >=
      textOffset
    );
  });
  return [
    ...
    marker?.index === undefined
      ? undefined
      : visibleWidth(stripTerminalSequences(rendered.slice(0, marker.index))) -
        textOffset,
  ];
}
...
describe("working-line extension lifecycle integration", () => {
  it("wires full-row rebuilds, authoritative usage, parallel tools, and isolated cleanup", async () => {
    ...
    expect(
      current.calls.slice(0, 2).map(([name, value]) => [name, value]),
    ).toEqual([
      ["message", ""],
    ...
    const laterPartialAssistant = {
      ...partialAssistant,
      usage: { input: 120, output: 7 },
    };
    const finalAssistant = {
      ...partialAssistant,
      usage: { input: 150, output: 9 },
    };
    ...
    await emit(handlers, "tool_execution_end", current.ctx, {
      toolCallId: "two",
    });
    expect(row()).toMatch(/Stable · read · \d+s · ↑150 ↓9/);
    await emit(handlers, "tool_execution_end", current.ctx, {
    ...
    expect(current.calls.filter(([name]) => name === "message")).toEqual([
      ["message", ""],
    ]);
    ...
    expect(
      current.calls.slice(-2).map(([name, value]) => [name, value]),
    ).toEqual([
      ["indicator", undefined],
      ["message", undefined],
    ]);
      ...
      current.calls
        ...
        .map(([, value]) =>
          stripTerminalSequences(
            (value as { frames?: string[] }).frames?.[0] ?? "",
          ),
        );
    const message = (
      input: number,
  ...
  it.each(["star-bloom", "pulse"] as const)(
    ...
    async (spinner) => {
      ...
      try {
        ...
        expect(
          current.calls.filter(([name]) => name === "indicator"),
        ).toHaveLength(2);
        expect(loaderPhase(loader.render(80)[1] ?? "")).toEqual(
          loaderPhase(activatedAtFrameZero),
        );
        ...
        expect(
          beforePhase[1],
          "expected animated-text high tier before rebuild",
        ).toBeDefined();
        const assistant = {
          role: "assistant",
        ...
        expect(
          afterPhase[1],
          "expected animated-text high tier after rebuild",
        ).toBeDefined();
  ...
  it("owns the full fallback row while custom messages are off and releases both APIs", async () => {
    ...
    expect(stripTerminalSequences(indicator.frames?.[0] ?? "")).toContain(
      "Working…",
    );
    await emit(handlers, "session_shutdown", current.ctx);
