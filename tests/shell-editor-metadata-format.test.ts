...
} from "../extensions/shell/editor-metadata-format";
...
function render(
  format: string,
  overrides: Partial<EditorMetadataValues> = {},
): string {
...
describe("renderEditorMetadataFormat", () => {
  ...
  it("uses conditional groups for optional values", () => {
    expect(render("$model( · $thinking)", { thinking: "off" })).toBe(
      "[accent]Model Label",
    );
    expect(render("$model( · $thinking)")).toBe(
      "[accent]Model Label[border] · [muted]high",
    );
    expect(
      render("$model( · $model_name)( · $session_name)", {
        modelName: "",
        sessionName: "",
      }),
    ).toBe("[accent]Model Label");
    expect(render("before((literal))after")).toBe(
      "[border]before[border]literal[border]after",
    );
  });

  it("renders unknown variables and fill as empty", () => {
    expect(render("a$unknown-b$fill-c($unknown)($fill)")).toBe(
  ...
  it.each([
    ...
    [
      { editorThinking: "thinkingText", editorThinkingXhigh: "thinkingXhigh" },
      "thinkingXhigh",
    ],
    [{ editorThinking: "thinkingText" }, "thinkingText"],
  ] as const)(
    "resolves max thinking through its compatibility chain",
    (colors, expected) => {
      const config = {
        ...defaultConfig,
...
describe("sanitizeEditorMetadataText", () => {
  it("removes SGR and BEL while preserving regular spaces", () => {
    expect(
      sanitizeEditorMetadataText("a  b\r\n\tc\u0000\u0007\u001b[31md\u001b[0m"),
    ).toBe("a  b cd");
  });
  ...
  it("resumes after CAN or SUB cancels ESC-prefixed control sequences", () => {
    expect(sanitizeEditorMetadataText("safe\u001b]8;;url\u0018 visible")).toBe(
      "safe visible",
    );
    expect(sanitizeEditorMetadataText("safe\u001bPpayload\u001a visible")).toBe(
      "safe visible",
    );
    expect(sanitizeEditorMetadataText("safe\u001b[31\u0018 visible")).toBe(
      "safe visible",
    );
    expect(sanitizeEditorMetadataText("safe\u001b[31\u001a visible")).toBe(
      "safe visible",
  ...
  it("resumes after CAN or SUB cancels C1 control sequences", () => {
    ...
    expect(sanitizeEditorMetadataText("safe\u0090payload\u0018 visible")).toBe(
      "safe visible",
    );
    expect(sanitizeEditorMetadataText("safe\u009b31\u0018 visible")).toBe(
      "safe visible",
  ...
  it("drops unterminated control sequences conservatively", () => {
    expect(
      sanitizeEditorMetadataText("safe\u001b]8;;https://example.com/unsafe"),
    ).toBe("safe");
  ...
  it("normalizes NEL, line separators, and paragraph separators", () => {
    expect(sanitizeEditorMetadataText("a\u0085\u2028\u2029b  c")).toBe(
      "a b  c",
    );
  });
});
