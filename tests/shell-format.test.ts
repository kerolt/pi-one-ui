...
function makeAssistantEntry(
  ...
  return makeMessageUsageEntry(
    "assistant",
    makeUsage(input, output, cost, cacheRead, cacheWrite),
  );
}
...
describe("usage formatting", () => {
  ...
  it("falls back to the active branch when getEntries is unavailable", () => {
    const ctx = {
      sessionManager: { getBranch: () => [makeAssistantEntry(12, 3, 0.4)] },
    };

    expect(getUsageTotals(ctx as never)).toMatchObject({
      input: 12,
      output: 3,
      cost: 0.4,
    });
...
describe("buildSessionDurationLabel", () => {
  ...
  it("formats minutes and seconds", () => {
    expect(buildSessionDurationLabel(FIXED_NOW - (2 * 60 + 13) * 1000)).toBe(
      "2m 13s",
    );
  });
  ...
  it("formats hours and minutes", () => {
    expect(
      buildSessionDurationLabel(FIXED_NOW - (1 * 3600 + 5 * 60) * 1000),
    ).toBe("1h 5m");
  });
...
describe("formatGitBranchText", () => {
  it("preserves full, under-limit, and exactly-at-limit branch names", () => {
    expect(formatGitBranchText("feature/long-name", "full")).toBe(
      "feature/long-name",
    );
...
describe("context helpers", () => {
  ...
  it("builds stable-width gauges and style labels", () => {
    ...
    expect(
      buildContextDisplayLabel({ percent: null, contextWindow: 128_000 }),
    ).toBe("?/128k");
    expect(
      buildContextDisplayLabel({ percent: Number.NaN, contextWindow: 128_000 }),
    ).toBe("?/128k");
    expect(
      buildContextDisplayLabel({ percent: null, contextWindow: undefined }),
    ).toBe("--");
  });
});
...
describe("getUsageTotals caching", () => {
  ...
  it.each([
    [
      "assistant",
      () => makeMessageUsageEntry("assistant", makeUsage(1, 1, 0.1)),
    ],
    [
      "tool result",
      () => makeMessageUsageEntry("toolResult", makeUsage(1, 1, 0.1)),
    ],
    [
      "compaction",
      () => makeSummaryUsageEntry("compaction", makeUsage(1, 1, 0.1)),
    ],
    [
      "branch summary",
      () => makeSummaryUsageEntry("branch_summary", makeUsage(1, 1, 0.1)),
    ],
    ...
    const entry = entries[1] as {
      message?: { usage?: TestUsage };
      usage?: TestUsage;
    };
