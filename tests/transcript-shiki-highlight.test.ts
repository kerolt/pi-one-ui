...
import {
  MAX_HL_CHARS,
  ShikiHighlightCache,
} from "../extensions/transcript/renderer/tool/diff/shiki-highlight.ts";
import { shouldHighlightCodeBlock } from "../extensions/transcript/renderer/tool/diff/diff-renderer.ts";
...
test("Shiki cache deduplicates pending work, caches success, and isolates themes", async () => {
  ...
  assert.equal(
    cache.get("const x = 1", "ts", "github-light", ["fallback"]),
    undefined,
  );
  await settle();
...
test("Shiki failures degrade without caching, retry, and oversized blocks are skipped", async () => {
  ...
  assert.equal(
    cache.get(oversized, "ts", "github-dark", ["fallback"]),
    undefined,
  );
