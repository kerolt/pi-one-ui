...
test("expanded ccstyle tools use Pi's native background card", async () => {
  ...
  try {
    ...
    assert.equal(
      component.children.includes(component.selfRenderContainer),
      true,
    );
...
test("MCP detection, titles, details, and custom tools use the global wrapper", async () => {
  ...
  assert.equal(
    humanizeMcpToolName("mcp__filesystem__read_file"),
...
test("global renderer reload chains external wrappers and shutdown restores them", async () => {
  ...
  try {
    ...
    for (const name of methodNames)
      assert.notEqual(prototype[name], external[name]);
    ...
    for (const name of methodNames)
      assert.equal(
        externalCalls[name],
        1,
        `${name} chains the external wrapper`,
      );
    ...
    for (const name of methodNames) {
      assert.equal(
        prototype[name],
        external[name],
        `shutdown restores ${name}'s downstream`,
      );
    }
  } finally {
    await firstEvents.get("session_shutdown")?.({}, ctx);
...
test("global renderer migrates legacy Symbol state without retaining old wrappers", async () => {
  ...
  try {
    ...
    for (const name of methodNames)
      assert.equal(migrated.downstream[name], originals[name]);
