...
describe("Accent Rail fullscreen layout patch", () => {
  it("clones one owned active entry with a stable forwarding component", () => {
    ...
    const entries = [
      untouched,
      { component: container, shrink: 1, minSize: 3, basis: "auto" },
    ];
    const node = { type: "vstack", entries, gap: 1, align: "stretch" };

    const adjusted = adjustAccentRailLayoutNode(
      node,
      new Set([owner]),
    ) as typeof node;
    const adjustedAgain = adjustAccentRailLayoutNode(
      node,
      new Set([owner]),
    ) as typeof node;
    const wrapped = adjusted.entries[1]?.component as {
      render(): string[];
      invalidate(): void;
    };
    ...
    expect(adjusted.entries[1]).toMatchObject({
      shrink: 1,
      minSize: 3,
      basis: "auto",
    });
  ...
  it("fails open for multiple children, multiple matches, malformed nodes, and throwing markers", () => {
    ...
    const multipleChildren = {
      ...
      entries: [
        {
          component: { children: [first.children[0], {}] },
          shrink: 1,
          minSize: 3,
        },
      ],
    ...
    expect(adjustAccentRailLayoutNode(multipleChildren, new Set([owner]))).toBe(
      multipleChildren,
    );
    expect(adjustAccentRailLayoutNode(multipleMatches, new Set([owner]))).toBe(
      multipleMatches,
    );
    expect(adjustAccentRailLayoutNode(throwingNode, new Set([owner]))).toBe(
      throwingNode,
    );
  ...
  it("marks only the requested outer editor with a non-enumerable own marker", () => {
    ...
    expect(Object.keys(outer)).not.toContain(
      String(ZENTUI_ACCENT_RAIL_LAYOUT_EDITOR),
    );
  ...
  it("disposes a delayed installation when its lifecycle generation becomes stale", async () => {
    ...
    const pending = retainAccentRailLayoutPatchInstallation(
      ...
      () =>
        lifecycle.isCurrent(lifecycleGeneration) &&
        capturedSerial === installSerial,
      retained,
    );
    lifecycle.shutdown();
    installSerial += 1;
  ...
  it("responds to live style and enablement predicates without reinstalling", () => {
    ...
    const layout = () =>
      (stack as unknown as Record<symbol, () => { entries: Entry[] }>)[
        LAYOUT_NODE
      ]?.()?.entries[0];
  ...
  it("patches only the layout-node method and preserves regular rendering and intrinsic rows", () => {
    ...
    const node = (stack as unknown as Record<symbol, () => unknown>)[
      LAYOUT_NODE
    ]?.() as {
      entries: Entry[];
    };
    ...
    expect((entry.component as { render(): string[] }).render()).toEqual([
      "",
      "rail",
    ]);
  ...
  it("supports multiple owners and restores the exact predecessor descriptor after the last cleanup", () => {
    ...
    installation.cleanup();
    expect(
      Object.getOwnPropertyDescriptor(FakeVStack.prototype, method)?.value,
  ...
  it("preserves a successor and keeps a later-restored ownerless wrapper inert", () => {
    ...
    const zentuiWrapper = (
      FakeVStack.prototype as unknown as Record<symbol, LayoutMethod>
    )[method];
    const successor = function successorPatch(
      this: unknown,
      ...args: unknown[]
    ) {
    ...
    second.cleanup();
    expect(
      (FakeVStack.prototype as unknown as Record<symbol, unknown>)[method],
    ).toBe(successor);
  ...
  it("discovers a structurally compatible non-exported layout symbol", () => {
    ...
    const stack = new FakeVStack();
    stack.entries = [
      { component: markedContainer(owner).container, shrink: 1, minSize: 3 },
    ];
    const node = (stack as unknown as Record<symbol, () => unknown>)[
      alternate
    ]?.() as {
  ...
  it.skipIf(!localSupportsAccentRailPatch)(
    ...
    () => {
      ...
      expect(
        (originalEntry.component as { render(): string[] }).render(),
      ).toEqual(["", "rail"]);
      expect(
        (stack as unknown as { entries: Entry[] }).entries[0]?.minSize,
      ).toBe(3);
      installation.cleanup();
    },
  );
  ...
  it.skipIf(!localSupportsAccentRailPatch)(
    ...
    async () => {
      ...
      const target =
        await discoverAccentRailLayoutPatchTargetFromEntrypoint(entrypoint);
      expect(target?.version).toBe(localPiTuiVersion);
      // A flat installation has no nested host copy to discover.
      if (target?.resolvedModulePath === target?.localModulePath) return;
  ...
  it("canonicalizes an npm-bin symlink and patches only the distinct host VStack", async () => {
    ...
    try {
      ...
      const installation = target
        ...
        : undefined;
      expect(installation?.diagnostic).toBe("installed");
      const hostStack = Object.create(target?.prototype ?? null) as Record<
        PropertyKey,
        unknown
      >;
      hostStack.entries = [
      ...
      expect((hostEntry.component as { render(): string[] }).render()).toEqual([
        "",
        "rail",
      ]);

      if (typeof VStack === "function" && typeof Container === "function") {
        const localStack = new VStack([
          { component: new Container(), shrink: 1, minSize: 3 },
        ]);
        const localNode = (
          localStack as unknown as Record<symbol, () => { entries: Entry[] }>
        )[LAYOUT_NODE]?.();
      ...
      installation?.cleanup();
      const restoredNode = (
        hostStack[LAYOUT_NODE] as () => { entries: Entry[] }
      )();
  ...
  it("fails open for wrappers, directories, missing paths, and broken symlinks", async () => {
    ...
    try {
      ...
      expect(
        await discoverAccentRailLayoutPatchTargetFromEntrypoint(wrapper),
      ).toBeUndefined();
      expect(
        await discoverAccentRailLayoutPatchTargetFromEntrypoint(root),
      ).toBeUndefined();
      expect(
      ...
      expect(
        await discoverAccentRailLayoutPatchTargetFromEntrypoint(broken),
      ).toBeUndefined();
    } finally {
  ...
  it.skipIf(!optedInGlobalPi)(
    ...
    async () => {
      ...
      expect(target?.canonicalEntrypoint).toContain(
        "pi-coding-agent/dist/cli.js",
      );
      if (typeof VStack === "function")
        expect(target?.prototype).not.toBe(VStack.prototype);
      ...
      expect(installation?.diagnostic).toBe("installed");
      const { container } = markedContainer(owner);
      const receiver = Object.create(target?.prototype ?? null) as Record<
        PropertyKey,
        unknown
      >;
      ...
      if (!adjustedComponent)
        throw new Error("missing adjusted layout component");
      expect(node.entries[0]?.minSize).toBe(3);
      expect(adjustedComponent).not.toBe(container);
      expect((adjustedComponent as { render(): string[] }).render()).toEqual([
        "",
        "rail",
      ]);
      if (typeof VStack === "function" && typeof Container === "function") {
      ...
      if (!restoredComponent)
        throw new Error("missing restored layout component");
      expect(restored.entries[0]?.minSize).toBe(3);
      expect(restoredComponent).toBe(container);
      expect((restoredComponent as { render(): string[] }).render()).toEqual([
        "rail",
      ]);
    },
  );
...
function makeHostPackageFixture(useLocalTui = false): {
  ...
  if (useLocalTui) {
    const localEntry = createRequire(import.meta.url).resolve(
      "@earendil-works/pi-tui",
    );
