...
import {
  ...
  packCompactChunks,
  reflowFullFooter,
} from "../extensions/layouts/footer/footer-layout";
...
describe("responsive footer layout", () => {
  it("models centered-middle placement and its floor bias", () => {
    expect(
      fullFooterFitsAligned({ left: "LLLLL", middle: "MMMM", right: "" }, 10),
    ).toBe(false);
    expect(
      fullFooterFitsAligned({ left: "LLLLL", middle: "MMMM", right: "" }, 11),
    ).toBe(true);
    expect(
      fullFooterFitsAligned({ left: "", middle: "MMMM", right: "RRRRR" }, 10),
    ).toBe(true);
    expect(
      fullFooterFitsAligned({ left: "LLLL", middle: "MMM", right: "RRRR" }, 13),
    ).toBe(true);
    expect(
      fullFooterFitsAligned({ left: "LLLL", middle: "MMM", right: "RRRR" }, 12),
    ).toBe(false);
  });
  ...
  it("prefers left / middle+right and falls back to left+middle / right", () => {
    expect(
      reflowFullFooter({ left: "LLLL", middle: "MM", right: "RR" }, 5),
    ).toEqual(["LLLL", "MM RR"]);
    expect(
      reflowFullFooter({ left: "L", middle: "MMMM", right: "RRRR" }, 6),
    ).toEqual(["L MMMM", "RRRR"]);
    expect(
      reflowFullFooter({ left: "LLLLLL", middle: "MMMM", right: "RRRR" }, 6),
    ).toBeUndefined();
  });

  it("collapses empty reflow zones without blank rows or stray spaces", () => {
    expect(reflowFullFooter({ left: "L", middle: "", right: "R" }, 2)).toEqual([
      "L",
      "R",
    ]);
    expect(reflowFullFooter({ left: "", middle: "M", right: "R" }, 3)).toEqual([
      "M R",
    ]);
    expect(reflowFullFooter({ left: "L", middle: "M", right: "" }, 3)).toEqual([
      "L",
      "M",
    ]);
  });
  ...
  it("packs chunks with boundary-aware same-row joins and no divider at breaks", () => {
    expect(
      packCompactChunks([chunk("one"), chunk("two")], 7, 2, " | "),
    ).toEqual(["one two"]);
    expect(
      packCompactChunks([chunk("one"), chunk("two", "separator")], 9, 2, " | "),
    ).toEqual(["one | two"]);
    expect(
      packCompactChunks([chunk("one"), chunk("two", "separator")], 8, 2, " | "),
    ).toEqual(["one", "two"]);
    expect(
      packCompactChunks(
        [chunk("one"), chunk(""), chunk("  "), chunk("two")],
        7,
        2,
        " | ",
      ),
  ...
  it("retains the next surviving chunk's incoming boundary", () => {
    expect(
      packCompactChunks(
        [chunk("A"), chunk("", "separator"), chunk("B", "space")],
        5,
        2,
        " | ",
      ),
    ...
    expect(
      packCompactChunks(
        [chunk("A"), chunk("", "space"), chunk("B", "separator")],
        5,
        2,
        " | ",
      ),
  ...
  it("supports every finite line limit and unlimited rows", () => {
    ...
    expect(packCompactChunks(chunks, 7, 2, " | ")).toEqual([
      "one two",
      "three",
    ]);
    expect(packCompactChunks(chunks, 5, 3, " | ")).toEqual([
      "one",
      "two",
      "three",
    ]);
    expect(packCompactChunks(chunks, 5, "unlimited", " | ")).toEqual([
      "one",
      "two",
      "three",
    ]);
  ...
  it("keeps every ANSI and wide-glyph row within width, even below the compact budget", () => {
    ...
    expect(
      packCompactChunks([chunk("abcdefgh")], 5, 1, " | ").every(
        (row) => visibleWidth(row) <= 5,
      ),
    ).toBe(true);
  });
});
