...
const originalSettingsSelectorRenderDescriptor =
...
const originalSettingsSelectorRender =
  SettingsSelectorComponent.prototype.render;
    ...
    components: {
      ...
      footer: {
        ...
        styles: {
          starship: {
            ...starship,
            format: flatChanged("footerFormat")
              ? config.footerFormat
              : starship.format,
            ...
            separator: flatChanged("separator")
              ? config.separator
              : starship.separator,
            contextStyle: flatChanged("contextStyle")
              ? config.contextStyle
              : starship.contextStyle,
            contextThresholds: flatChanged("contextThresholds")
              ? config.contextThresholds
              : starship.contextThresholds,
            pathDisplay: flatChanged("pathDisplay")
              ? config.pathDisplay
              : starship.pathDisplay,
            segments: flatChanged("footerSegments")
              ? config.footerSegments
              : starship.segments,
            gitBranch: flatChanged("gitBranch")
              ? config.gitBranch
              : starship.gitBranch,
            gitCommit: flatChanged("gitCommit")
              ? config.gitCommit
              : starship.gitCommit,
            gitMetrics: flatChanged("gitMetrics")
              ? config.gitMetrics
              : starship.gitMetrics,
...
function installUserMessageStyle(
  ...args: Parameters<typeof installUserMessageStyleProduction>
) {
...
function installSelectorBorderStyle(
  ...
  return installSelectorBorderStyleProduction(
    ...
    args[1]
      ? () =>
          canonicalizeTestConfig(
            (args[1]?.() ?? defaultConfig) as PolishedTuiConfig,
          )
      : undefined,
  );
}

function patchSelectorBorderStyle(
  ...args: Parameters<typeof patchSelectorBorderStyleProduction>
) {
  return patchSelectorBorderStyleProduction(
    ...
    args[2]
      ? () =>
          canonicalizeTestConfig(
            (args[2]?.() ?? defaultConfig) as PolishedTuiConfig,
          )
      : undefined,
  );
}
...
class WrappedPolishedEditor extends WrappedPolishedEditorProduction {
  constructor(
    ...args: ConstructorParameters<typeof WrappedPolishedEditorProduction>
  ) {
    const getConfig = args[2];
    super(
      args[0],
...
function makeContext(overrides: Record<string, unknown> = {}) {
  ...
  const overrideUi =
    overrides.ui && typeof overrides.ui === "object" ? overrides.ui : undefined;
  return {
    hasUI: true,
    mode: "tui",
    cwd: process.cwd(),
...
afterAll(() => {
  ...
  if (isolatedAgentDir.previous === undefined)
...
afterEach(() => {
  ...
  restoreDescriptor(
    UserMessageComponent.prototype,
    "render",
    originalUserMessageRenderDescriptor,
  );
  restoreDescriptor(
    UserMessageComponent.prototype,
    "invalidate",
...
describe("fixed-editor retirement contract", () => {
  it("contains no retired implementation, package, CI, or documentation artifacts", () => {
    ...
    const retiredPaths = [
      ...
      [
        "test",
        [["fixed", "editor"].join("-"), "editor-layout.test.ts"].join("-"),
      ],
      ["test", [["fixed", "editor"].join("-"), "layout.test.ts"].join("-")],
      ["test", [["fixed", "editor"].join("-"), "test.ts"].join("-")],
      [
        "test",
        [["fixed", "editor"].join("-"), "terminal-state.test.ts"].join("-"),
      ],
    ...
    const tests = collectFiles(join(root, "tests"), (path) =>
      path.endsWith(".ts"),
    );
    for (const path of tests) {
      const source = readFileSync(path, "utf8");
      for (const token of testApiTokens)
        expect(source, `${path}: ${token}`).not.toContain(token);
    }
    ...
    if (existsSync(ciFile)) userFiles.push(ciFile);
    ...
    if (existsSync(docsRoot)) {
      userFiles.push(
        ...collectFiles(
          docsRoot,
          (path) => path.endsWith(".md") && path !== designRecord,
        ),
      );
    }
    ...
    expect(manifest.scripts).not.toHaveProperty(
      ["test", "fixed-editor"].join(":"),
    );
...
describe("Pi docs compliance", () => {
  ...
  it("installs enabled copy-friendly framed messages while the editor is disabled", async () => {
    ...
    expect(UserMessageComponent.prototype.render).not.toBe(
    ...
    expect(ModelSelectorComponent.prototype.render).not.toBe(
      originalModelSelectorRender,
    );
  ...
  it("installs only the editor and footer when message and selector surfaces are disabled", async () => {
    ...
    expect(new UserMessageComponent("message").render(20)).toEqual([
      "native:20",
    ]);
    expect(ModelSelectorComponent.prototype.render).toBe(
      originalModelSelectorRender,
    );
    await emit(handlers, "session_shutdown", ctx);
  });
...
`;
  ...
  it("isolates selector installation failure from editor, messages, and footer", async () => {
    const settingsPrototype =
      SettingsSelectorComponent.prototype as unknown as {
  ...
  it("does not install user-message rendering when ctx.hasUI is false", async () => {
    ...
    expect(UserMessageComponent.prototype.render).toBe(
      originalUserMessageRender,
    );
  });
        ...
        setEditorComponent(factory: unknown) {
          ...
          patchPresenceDuringReplacements.push(
            UserMessageComponent.prototype.render !==
              originalUserMessageRender ||
              ModelSelectorComponent.prototype.render !==
                originalModelSelectorRender,
          );
  ...
  it("keeps the active factory when expanded editor text cannot be read", async () => {
    ...
    expect(notifications.at(-1)).toContain(
      "expanded editor text could not be read safely",
    );
  });
  ...
  it("isolates user-message patch failure from editor and selector activation", async () => {
    ...
    try {
      ...
      expect(SettingsSelectorComponent.prototype.render).not.toBe(
        originalSettingsSelectorRender,
      );
      expect(UserMessageComponent.prototype.invalidate).toBe(
        originalUserMessageInvalidate,
      );
      expect(
        (
          UserMessageComponent.prototype as unknown as Record<
            PropertyKey,
            unknown
          >
        )[ZENTUI_PROTOTYPE_PATCH_REGISTRY],
      ...
      ui: {
        ...
        setEditorComponent(factory: unknown) {
          ...
          if (factory === existingFactory && !failedPreviousFactoryRestore) {
            ...
            throw new Error(
              "previous factory construction failed after assignment",
            );
          }
        },
        getEditorComponent: () => editorFactory,
  ...
  it("wraps an editor component already installed by another extension", async () => {
    ...
    const editor = (
      editorFactory as (
        ...args: unknown[]
      ) => ReturnType<typeof existingEditorFactory>
    )(
  ...
  it("cleans up when start and shutdown use distinct context wrappers", async () => {
    ...
    const startCtx = makeContext({
      ui: makeUiWrapper(() => startContextStale),
    });
    ...
    await expect(
      emit(handlers, "session_shutdown", shutdownCtx),
  ...
  it("keeps a newer instance installed when the older instance shuts down", async () => {
    ...
    expect(UserMessageComponent.prototype.invalidate).toBe(
      secondMessageInvalidate,
    );
    expect(ModelSelectorComponent.prototype.render).toBe(secondModelSelector);
    expect(SettingsSelectorComponent.prototype.render).toBe(
      secondSettingsSelector,
    );
    expectSinglePromptZone(
      new UserMessageComponent("new").render(30).join("\n"),
    );
  ...
  it("routes removed commands through the ordinary usage warning without mutation", async () => {
    ...
    expect(
      notifications.every((message) => message.startsWith("Usage: /zentui")),
    ).toBe(true);
    await emit(handlers, "session_shutdown", ctx);
  });
  ...
  it("renders editor and message style selections independently", () => {
    ...
    try {
      ...
      expect(editor.render(80).join("\n")).toContain("│");
      expect(
        new UserMessageComponent("message 2").render(80).join("\n"),
      ).toContain("User");
    } finally {
      cleanup();
    }
  ...
  it.each([
    ...
    "composes %s Editor with %s messages independently",
    (editorStyle, messageStyle) => {
      ...
      const messageRendered = new UserMessageComponent("message")
        .render(40)
        .join("\n");
  ...
  it("does not clobber selector lines that are not borders", () => {
    ...
    expect(prototype.render(8)).toEqual([
      "Selector title",
      "────────",
      "help text",
    ]);
  ...
  it("restores an own method descriptor exactly", () => {
    const predecessor = (width: number) => [
      "─".repeat(width),
      "body",
      "─".repeat(width),
    ];
  ...
  it("does not stack selector wrappers and ignores stale cleanup", () => {
    const predecessor = vi.fn((width: number) => [
      "─".repeat(width),
      "body",
      "─".repeat(width),
    ]);
  ...
  it("preserves a later selector replacement and its predecessor chain", () => {
    ...
    expect(prototype.render(4)).toEqual([
      "third-party",
      "────",
      "body",
      "────",
  ...
  it("deactivates an older selector record hidden inside a third-party predecessor chain", () => {
    ...
    prototype.render = thirdParty;
    const cleanupSecond = patchSelectorBorderStyle(
      prototype,
      secondTheme,
      () => defaultConfig,
    );
  ...
  it("user-message cleanup restores exact render and invalidate predecessors", () => {
    ...
    expect(UserMessageComponent.prototype.invalidate).not.toBe(
      predecessorInvalidate,
    );
  ...
  it.each(["framed", "framed-copy-friendly", "compact", "labeled"] as const)(
    ...
    (style) => {
      ...
      for (const width of [8, 40]) {
        const preserved = new UserMessageComponent(
          `before ${open}LINK${close}\nafter`,
        )
          .render(width)
          .join("\n");
  ...
  it("removes BEL, ST, and C1-ST terminated OSC 133 zones without removing user text", () => {
    ...
    const hostile =
      "before \x1b]133;A\x07middle \x1b]133;B\x1b\\after \x9d133;C\x9c final";
  ...
  it.each([
    [
      "unterminated ESC OSC",
      "before \x1b]133;Atail",
      "before tail",
      "\x1b]133",
    ],
  ...
  it.each([
    ...
    (_name, unrelated, hostile, openIntroducer) => {
      ...
      const rendered = new UserMessageComponent(
        `before ${unrelated}${hostile}after`,
      )
        .render(80)
        .join("\n");
  ...
  it.each([
    ...
    (_name, hostile, open, close, hostileFragment) => {
      ...
      const rendered = new UserMessageComponent(
        `before ${hostile}${open}link${close} after`,
      )
        .render(100)
        .join("\n");
      const visible = stripPromptMarks(rendered);
  ...
  it("uses sanitized plain output when styled rendering fails with source OSC", () => {
    ...
    const open = "\x1b]8;;https://raw.example\x07";
    const close = "\x1b]8;;\x07";
    const output = new UserMessageComponent(
      `before ${clipboard}${open}link${close} after`,
    )
      .render(80)
      .join("\n");
  ...
  it.each(["framed", "framed-copy-friendly", "compact", "labeled"] as const)(
    ...
    (style) => {
      ...
      const lines = new UserMessageComponent("hello ".repeat(20))
        .render(12)
        .map(stripPromptMarks);
  ...
  it("reuses user-message wrappers while stale cleanup leaves the new registration active", () => {
    ...
    firstCleanup();
    const secondRender = new UserMessageComponent("hello")
      .render(80)
      .join("\n");
  ...
  it("deactivates an older user-message record hidden inside a third-party predecessor chain", () => {
    ...
    prototype.render = thirdParty;
    const cleanupSecond = installUserMessageStyle(
      secondTheme,
      () => defaultConfig,
    );
    ...
    expect(
      prototype.render.call({ children: [{ text: "hello" }] } as never, 12),
    ).toEqual(["third-party", "base:12"]);
    expect(firstTheme).not.toHaveBeenCalled();
    expect(secondTheme).not.toHaveBeenCalled();
  });
  ...
  it("keeps the production footer visible while the minimalist editor decorates", async () => {
    ...
    const nativeEditorPrototype = Object.getPrototypeOf(
      PolishedEditor.prototype,
    ) as {
      render(width: number): string[];
    };
    const nativeRender = vi
      .spyOn(nativeEditorPrototype, "render")
      .mockImplementation(() => {
  ...
  it("keeps the production footer for unsupported wrapped minimalist output", async () => {
    ...
    const editor = (
      editorFactory as (
        ...args: unknown[]
      ) => ReturnType<typeof existingFactory>
    )(
      { requestRender() {}, terminal: { rows: 24, cols: 80 } } as never,
      { borderColor: (text: string) => text, selectList: {} } as never,
      {} as never,
  ...
  it("forces an initial project refresh for minimalist mode without a status line or polling", async () => {
    ...
    writeFileSync(
      join(cwd, ".git", "HEAD"),
      "ref: refs/heads/minimalist-test\n",
    );
    writeFileSync(
      join(cwd, ".git", "config"),
  ...
  it("composes telemetry only into built-in wide segments and explicit atomic variables", () => {
    ...
    installFooter(
      ctx as never,
      state,
      () => ({ ...defaultConfig, responsiveFooter: false }),
      {
        setRequestRender() {},
        scheduleProjectRefresh() {},
      },
    ...
    expect(custom).toContain(
      "↑100 ↓20 󰆼 80.0% R1.2k W300 $1.000 (sub) 0.5%/200k (auto)",
    );
  ...
  it("renders opt-in model info, independent variables, and omits it from compact fallback", () => {
    ...
    for (const [format, expected, omitted] of [
      ...
      if (format.includes("$fill")) {
        expect(custom.indexOf("Provider X")).toBeLessThan(
          custom.indexOf("custom-model"),
        );
      }
    }
  ...
  it.each([
    ...
    (separator, expectedSeparator) => {
      ...
      expect(rendered).toContain(
        ["A", "B", "0.5%/200k", "tokens", "cost"].join(expectedSeparator),
      );
    },
  );
  ...
  it("styles third-party statuses with colors.extensionStatus", () => {
    ...
    installFooter(
      ctx as never,
      state,
      () => configWithColors({ extensionStatus: "warning" }),
      {
        setRequestRender() {},
        scheduleProjectRefresh() {},
      },
  ...
  it("selects aligned, full reflow, and compact stages at exact target widths", () => {
    ...
    const ctx = makeContext({
      ...
      sessionManager: {
        getBranch: () => [],
        getSessionName: () => "responsive-layout-long",
      },
      ui: {
        theme: makeTheme(),
        setFooter(factory: FooterFactory | undefined) {
  ...
  it("toggles the editor from direct Zentui slash-command arguments", async () => {
    ...
    expect(notifications).toEqual([
      { message: "Editor: disabled", level: "info" },
    ]);
  });
  ...
  it("renders Zentui settings with mode-aware top and bottom borders", async () => {
    ...
    expect(
      themeLines.every(
        (line) => visibleWidth(stripTestTags(line)) <= settingsWidth,
      ),
    ).toBe(true);
    ...
    expect(
      terminalLines.every(
        (line) => visibleWidth(stripPromptMarks(line)) <= settingsWidth,
      ),
    ).toBe(true);
  });
  ...
  it("cycles branch length presets and returns custom JSON values to full", async () => {
    const run = async (
      maxLength: PolishedTuiConfig["gitBranch"]["maxLength"],
      presses: number,
    ) => {
      let command:
        | { handler: (args: string, ctx: unknown) => Promise<void> }
  ...
  it("shows selector color sources independently", async () => {
    ...
    registerZentuiSettingsCommand(
      ...
      } as never,
      {
        ...
        sessionLifecycle: inactiveSessionLifecycle,
        getConfig: () =>
          configWithColorSources({ editor: "theme", userMessages: "terminal" }),
        setSelectorBordersComponent(patch) {
          changes.push(patch);
        },
        setFooterSegments() {},
  ...
  it("cycles active third-party status placement from the extension segments tab", async () => {
    ...
    registerZentuiSettingsCommand(
      ...
      } as never,
      {
        ...
        setSeparator() {},
        getActiveExtensionStatuses: () =>
          new Map<string, string>([["alpha", "ok"]]),
        setExtensionStatusPlacement(key, placement) {
          placements.push({ key, placement });
        },
        setExtensionStatusColorMode() {},
  ...
  it("does not show inactive saved placements in the extension segments tab", async () => {
    ...
    registerZentuiSettingsCommand(
      ...
      } as never,
      {
        ...
        setSeparator() {},
        getActiveExtensionStatuses: () =>
          new Map<string, string>([["active", "ok"]]),
        setExtensionStatusPlacement() {},
        setExtensionStatusColorMode() {},
        requestRender() {},
        settingsListTheme: {
  ...
  function createSessionNameFooter({
    ...
    installFooter(
      ctx as never,
      createInitialState({ ...emptyGitStatus(), branch }),
      () => config,
      {
        setRequestRender() {},
        scheduleProjectRefresh() {},
  ...
  it("omits absent names and keeps Unicode names within narrow footer widths", () => {
    ...
    const lines = renderSessionNameFooter({
      name: "研究 🚀 ".repeat(20),
      width: 18,
    });
    expect(lines.every((line) => visibleWidth(line) <= 18)).toBe(true);
  ...
  it("reads an updated session name on the next footer render", () => {
    ...
    const footer = createSessionNameFooter({
      getSessionName: () => sessionName,
    });
    expect(footer?.render(120).join("\n")).toContain("draft");
  ...
  it("preserves an owned Starship Footer when a live Hidden replacement fails", async () => {
    ...
    try {
      ...
      const ui = {
        ...
        getEditorComponent: () => undefined,
        setFooter(next: FooterFactory | undefined) {
          if (rejectReplacement && next !== undefined)
  ...
  it("preserves an owned Starship Footer when a live Native transition fails", async () => {
    ...
    try {
      ...
      const ui = {
        ...
        getEditorComponent: () => undefined,
        setFooter(next: FooterFactory | undefined) {
          if (rejectNative && next === undefined)
