...
} from "../extensions/transcript/renderer/compact-mode.ts";
import { refreshMountedTranscript } from "../extensions/transcript/renderer/transcript-refresh.ts";
import claudeCodeStyleExtension from "../extensions/transcript/renderer/index.ts";
import {
  getMessageDisplayTheme,
  setMessageDisplayTheme,
} from "../extensions/transcript/renderer/tool/message-display.ts";
import { WriteExecutionMetadataStore } from "../extensions/transcript/renderer/tool/diff/write-execution.ts";
import {
  invalidateIoView,
  isExpandedToolIoView,
} from "../extensions/transcript/renderer/tool/result.ts";
import { toolCallSummary } from "../extensions/transcript/renderer/tool/names.ts";
...
test("buildMessageSummary: duration first, read dedup by path, counts, first-seen order, edit/write excluded", () => {
  ...
  assert.equal(
    buildMessageSummary(message, query),
...
test("dim thinking text uses the dim token without mutating the theme", () => {
  ...
  try {
    ...
    assert.equal(
      styleCompactThinkingText("hi", theme as any),
      "<thinkingText>hi",
    );
...
test("consecutive tool-call messages accumulate into one round until the next visible assistant text", () => {
  ...
  try {
    ...
    assert.equal(
      edit.expanded,
      false,
      "edit/write keep independent expansion state",
    );
    ...
    assert.equal(
      cardLines[0],
      "",
      "expanded round keeps the normal card spacer",
    );
    assert.ok(
...
test("compact edit/write keeps the stats header and inherits on-mode diff limits", () => {
  ...
  try {
    ...
    assert.notEqual(
      expandedRich,
      collapsedRich,
      "expanded bakes a separate rich diff",
    );
    edit.render(120);
    assert.equal(
      edit.resultRendererComponent,
      expandedRich,
      "expanded rich diff is reused",
    );
    ...
    assert.match(
      titlePlain,
      /^ ✓ edit a\.ts/,
      "expanded title uses Box pad only",
    );
    ...
    assert.equal(
      isExpandedToolIoView(unknownEdit.resultRendererComponent),
      true,
    );
    ...
    assert.equal(
      isExpandedToolIoView(unknownWrite.resultRendererComponent),
      true,
    );
...
test("isCompactAssistantComponent gates on compact mode; setExpanded no-ops outside", () => {
  ...
  try {
    ...
    assert.equal(
      updates,
      before,
      "setExpanded is a no-op outside compact mode",
    );
