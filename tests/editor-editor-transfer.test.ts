...
import { replaceEditorComponentWithExpandedText } from "../extensions/surfaces/editor/editor-transfer";
...
describe("safe editor component transfer", () => {
  ...
  it.each(["ordinary prompt", ""])(
    "transfers normal prompt text: %j",
    (text) => {
      const operations: string[] = [];
      const ui = piLikeUi(textEditor(text), operations);
      const next = textEditor();

      expect(replaceEditorComponentWithExpandedText(ui, () => next)).toEqual({
        ok: true,
      });
  ...
  it("does not replace when the public transfer API is unavailable", () => {
    ...
    expect(
      replaceEditorComponentWithExpandedText({ setEditorComponent }, undefined),
    ).toEqual({
      ok: false,
  ...
  it("does not replace when the previous factory cannot be snapshotted", () => {
    ...
    expect(result).toEqual({
      ok: false,
      reason: "editor-factory-snapshot-failed",
    });
  ...
  it("does not replace when expanded text snapshotting fails", () => {
    ...
    expect(result).toEqual({
      ok: false,
      reason: "editor-text-snapshot-failed",
    });
  ...
  it("does not replace when writing expanded text back fails", () => {
    ...
    expect(result).toEqual({
      ok: false,
      reason: "editor-text-preparation-failed",
    });
    expect(setEditorComponent).not.toHaveBeenCalled();
  });
    ...
    const result = replaceEditorComponentWithExpandedText(
      {
        ...
        setEditorComponent(factory) {
          ...
          setterCalls += 1;
          throw new Error(
            setterCalls === 1 ? "replacement failed" : "rollback failed",
          );
        },
      },
      () => textEditor("next"),
