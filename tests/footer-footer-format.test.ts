...
import { FOOTER_FORMAT_ALIASES } from "../extensions/app/config/shell";
import {
...
} from "../extensions/layouts/footer/footer-format";
...
describe("parseFooterFormat", () => {
  ...
  it("parses $package and $package_version as package-version vars", () => {
    expect(parseFooterFormat("$package")).toEqual([
      { kind: "var", name: "package" },
    ]);
  ...
  it("parses $git_commit, $commit, $git_tag, and $tag as git-commit vars", () => {
    expect(parseFooterFormat("$git_commit")).toEqual([
      { kind: "var", name: "git_commit" },
    ]);
    expect(parseFooterFormat("$commit")).toEqual([
      { kind: "var", name: "commit" },
    ]);
    expect(parseFooterFormat("$git_tag")).toEqual([
      { kind: "var", name: "git_tag" },
    ]);
    expect(parseFooterFormat("$tag")).toEqual([{ kind: "var", name: "tag" }]);
  });

  it("parses git-metrics vars", () => {
    expect(parseFooterFormat("$git_metrics")).toEqual([
      { kind: "var", name: "git_metrics" },
    ]);
    expect(parseFooterFormat("$git_added")).toEqual([
      { kind: "var", name: "git_added" },
    ]);
    expect(parseFooterFormat("$git_deleted")).toEqual([
      { kind: "var", name: "git_deleted" },
    ]);
  });

  it("parses braced variables", () => {
    const braced = "${" + "git_branch}";
    expect(parseFooterFormat(braced)).toEqual([
...
describe("renderFormatSplit", () => {
  ...
  it("renders session-name variables in plain and braced forms", () => {
    expect(
      renderFormatSplit(parseFooterFormat("$session_name"), renderVar).left,
    ).toBe("Session name");
    const bracedSessionName = "${" + "session_name}";
    expect(
      renderFormatSplit(parseFooterFormat(bracedSessionName), renderVar).left,
    ).toBe("Session name");
  });
...
describe("conditional groups", () => {
  ...
  it("drops a conditional session-name group when the name is empty", () => {
    ...
    const raw = renderFormatSplit(
      parseFooterFormat("($sep$session_name)"),
      noSessionName,
    ).left;
...
describe("compact footer format", () => {
  it("splits top-level wrap variables and records incoming boundary kinds", () => {
    const expected = [
      {
        kind: "tokens",
        tokens: [{ kind: "var", name: "cwd" }],
        boundary: "space",
      },
      {
        kind: "tokens",
        tokens: [{ kind: "var", name: "context" }],
        boundary: "space",
      },
    ];
    expect(
      compileCompactFormat(parseFooterFormat("$cwd$wrap$context")),
    ).toEqual(expected);
    expect(
      compileCompactFormat(parseFooterFormat("$cwd$" + "{wrap}$context")),
    ).toEqual(expected);
  ...
  it("ignores fill and recognizes only a standalone extensions chunk", () => {
    const chunks = compileCompactFormat(
      parseFooterFormat(
        "$cwd$fill$wrap $extensions $wrap prefix $extensions$wrap($extensions)",
      ),
    );
  ...
  it("renders conditional chunks and preserves incoming boundaries around empty content", () => {
    const chunks = compileCompactFormat(
      parseFooterFormat(
        "$cwd$wrap_sep$missing$wrap(on $git_branch) $git_status$wrap_sep$context",
      ),
    );
  ...
  it("collects canonical data references and excludes layout variables", () => {
    expect(
      ...
      new Set(["cwd", "git_branch", "model", "provider", "session_duration"]),
    );
  });
});
...
describe("stripOrphanSeparators", () => {
  ...
  it("handles ANSI-wrapped themed separators", () => {
    ...
    expect(stripOrphanSeparators(`ctx${sep}${sep}tokens`)).toBe(
      `ctx${sep}tokens`,
    );
    expect(stripOrphanSeparators(`ctx${sep}tokens${sep}`)).toBe(
      `ctx${sep}tokens`,
    );
    expect(stripOrphanSeparators(`${sep}tokens${sep}cost`)).toBe(
      `tokens${sep}cost`,
    );
  });
