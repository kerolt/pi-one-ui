 ...
 */
function readTomlSectionKeyValue(
  raw: string,
  section: string,
  key: string,
): string | undefined {
...
const parsers: readonly ManifestSource[] = [
  ...
  // maven — pom.xml: project/version (direct child, simple case).
  {
    ...
    parse: (raw) => {
      const match = raw.match(
        /<project\b[^>]*>[\s\S]*?<version>\s*([^<]+?)\s*<\/version>/i,
      );
  ...
  // python — pyproject.toml ([project].version or [tool.poetry].version)
  {
    ...
    parse: (raw) => {
      const project = cleanVersion(
        readTomlSectionKeyValue(raw, "project", "version"),
      );
      if (project) return project;
      return cleanVersion(
        readTomlSectionKeyValue(raw, "tool.poetry", "version"),
      );
    },
  },
  // python — setup.cfg ([metadata].version)
  {
    files: ["setup.cfg"],
    parse: (raw) =>
      cleanVersion(readTomlSectionKeyValue(raw, "metadata", "version")),
  },
  // rust — Cargo.toml ([package].version, with workspace inheritance)
  {
    ...
    parse: (raw) => {
      ...
      if (
        directClean &&
        !/^\s*version\s*\.\s*workspace\s*=\s*true/i.test(direct ?? "")
      ) {
        return directClean;
      }
      ...
      const workspaceMatch = raw.match(
        /\[workspace\][\s\S]*?version\s*=\s*["']([^"']+)["']/i,
      );
  ...
  // fortran — fpm.toml: [package] or [project] version.
  {
    files: ["fpm.toml"],
    parse: (raw) => {
      const fromPackage = cleanVersion(
        readTomlSectionKeyValue(raw, "package", "version"),
      );
