...
const ADMONITION =
  /^>\s*\[\s*!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\s*\]\s*(.*)$/i;
...
function renderAdmonition(
  ...
  return {
    output: [
      `> **${style.icon} ${style.label}**${content ? ` ${content}` : ""}`,
      "",
    ],
    next: j,
  };
...
function trimUrl(url: string): string {
  ...
  for (;;) {
    if (
      t.endsWith(")") &&
      (t.match(/\(/g)?.length ?? 0) < (t.match(/\)/g)?.length ?? 0)
    ) {
      t = t.slice(0, -1);
    } else if (
      t.endsWith("]") &&
