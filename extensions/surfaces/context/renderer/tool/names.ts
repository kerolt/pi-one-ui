...
export function toolCallSummary(
  ...
  if (
    variant === "default" &&
    AGENT_FAMILY_TOOL_NAMES.has(toolName) &&
    args.agent_id
  ) {
  ...
  if (variant === "grouping" && (name === "agent" || name === "agents")) {
    ...
    return {
      main: value(
        name === "agent" ? "launch agent" : "launch agents",
        "description",
        "prompt",
      ),
      detail: "",
  ...
  if (
    variant === "grouping" &&
    (name === "get_subagent_result" || name === "steer_subagent")
  ) {
    return {
      main: value(
        name === "get_subagent_result" ? "agent result" : "steer agent",
        "agent_id",
      ),
      detail: "",
    };
  ...
  if (name === "taskcreate")
    return { main: value("create task", "subject"), detail: "" };
  ...
  if (variant === "grouping") {
    if (toolName === "bash")
      return { main: `Bash ${clip(args.command || "...")}`, detail: "" };
    if (toolName === "grep") {
