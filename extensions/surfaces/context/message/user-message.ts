...
} from "../../../shell/prototype-patch-registry.ts";
import {
...
function withPromptZoneMarkers(lines: string[]): string[] {
  if (lines.length === 1) {
    return [
      `${OSC133_ZONE_START}${lines[0]}${OSC133_ZONE_END}${OSC133_ZONE_FINAL}`,
    ];
  }
...
function sanitizePredecessorRender(result: unknown): unknown {
  if (typeof result === "string")
    return sanitizeRenderedUserMessageText(result);
  if (!Array.isArray(result)) return result;
  const stringRows = result.every(
