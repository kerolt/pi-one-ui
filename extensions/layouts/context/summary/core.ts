...
function summaryParts(data: AgentSummaryData): string[] {
  const parts: string[] = [];
  if (data.commands)
    parts.push(`ran ${data.commands} command${plural(data.commands)}`);
  ...
  if (data.writes)
    parts.push(`wrote ${data.writes} file${plural(data.writes)}`);
  if (data.others)
    parts.push(`${data.others} other tool${plural(data.others)}`);
...
export function summaryMarkdown(
  ...
  const capitalizeFirst = (part: string, first: boolean) => {
    ...
    return first && verb
      ? verb[0].toUpperCase() + verb.slice(1) + part.slice(verb.length)
      : part;
  };
  ...
  const text = parts
    ...
    .map((part) =>
      paintNumber(
        part.endsWith("failed") ? colors.failed : colors.success,
        part,
      ),
    )
    .join(", ");
