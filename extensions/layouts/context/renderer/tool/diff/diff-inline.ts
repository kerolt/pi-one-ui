...
export function buildSplitRows(entries: ParsedDiffEntry[]): SplitDiffRow[] {
  ...
  while (index < entries.length) {
    ...
    if (entry.lineKind === "remove") {
      const removedResult = collectConsecutiveLineEntries(
        entries,
        index,
        "remove",
      );
      const removed = removedResult.collected;
      const addedResult = collectConsecutiveLineEntries(
        entries,
        removedResult.nextIndex,
        "add",
      );
...
export function getCellLineNumber(
  line: DiffLineEntry,
  side: "left" | "right",
): number | null {
  if (side === "left") {
...
export function buildInlineHighlightMap(
  rows: SplitDiffRow[],
