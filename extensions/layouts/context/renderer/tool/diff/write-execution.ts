...
async function capturePreviousContent(
  absolutePath: string,
): Promise<WriteExecutionMeta> {
  let info;
  try {
  ...
  try {
    ...
    const previousContent = new TextDecoder("utf-8", { fatal: true }).decode(
      bytes,
    );
