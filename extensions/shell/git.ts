...
export function detectGitState(paths: GitStatePaths): {
  ...
  if (paths.rebaseMerge || paths.rebaseApply) {
    ...
    if (msgnum && end) {
      return {
        gitState: "REBASING",
        gitStateLabel: `REBASING ${msgnum}/${end}`,
      };
    }
  ...
  if (paths.revertHead)
    return { gitState: "REVERTING", gitStateLabel: "REVERTING" };
  if (paths.bisectLog)
    return { gitState: "BISECTING", gitStateLabel: "BISECTING" };
  return {};
}
...
async function readGitOperationState(cwd: string): Promise<{
  ...
  const existing = (path: string | undefined) =>
...
function isNotARepoError(error: unknown): boolean {
  const message =
    error instanceof Error
      ...
      : String(error);
  return /not a git repository|outside repository|not a git repo/i.test(
    message,
  );
}
...
export async function readGitStatus(
  ...
  try {
    ...
    const [{ stdout: statusStdout }, stashResult, tagResult, metricsResult] =
      await Promise.all([
        ...
        readExactTag
          ? execFileAsync(
              ...
              (r) => ({
                stdout:
                  typeof r.stdout === "string" ? r.stdout : String(r.stdout),
              }),
              () => ({ stdout: "" }),
            )
    ...
    const stashCount = stashStdout
      .split(/\r?\n/)
      .filter((line) => line.trim().length > 0).length;
    const status = parseGitStatusPorcelain(stdoutText, stashCount);
    if (status.commit) {
      const tagStdout =
        typeof tagResult.stdout === "string"
          ? tagResult.stdout
          : String(tagResult.stdout);
