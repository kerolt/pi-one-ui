...
export const ZENTUI_ACCENT_RAIL_LAYOUT_EDITOR = Symbol.for(
  "pi-zentui.accent-rail-layout-editor",
);
const ZENTUI_ACCENT_RAIL_LAYOUT_REGISTRY = Symbol.for(
  "pi-zentui.accent-rail-layout-registry",
);
...
function forwardingLayoutComponent(
  component: object,
...
function ownMarker(value: unknown): AccentRailEditorMarker | undefined {
  ...
  return marker &&
    typeof marker.owner === "symbol" &&
    typeof marker.active === "function"
    ? marker
    : undefined;
}
...
export function adjustAccentRailLayoutNode(
  ...
  if (candidate.type !== "vstack" || !Array.isArray(candidate.entries))
    return node;
  ...
  if (!matchedEntry?.component || typeof matchedEntry.component !== "object")
    return node;
...
function restorePredecessor(
  ...
  if (registry.predecessorDescriptor) {
    Object.defineProperty(
      prototype,
      registry.method,
      registry.predecessorDescriptor,
    );
  } else {
...
export function installAccentRailLayoutPatchOnTarget(
  ...
  const predecessorDescriptor = Object.getOwnPropertyDescriptor(
    prototype,
    method,
  );
  const predecessor = prototype[method];
  if (
    typeof predecessor !== "function" ||
    predecessorDescriptor?.configurable === false
  ) {
    return {
      cleanup: () => {},
      diagnostic: "unsupported-shape",
  ...
  registry.wrapper = function accentRailLayoutNodePatch(
    this: unknown,
    ...args: unknown[]
  ) {
...
export async function discoverAccentRailLayoutPatchTargetFromEntrypoint(
  ...
  try {
    ...
    const packageLocation = packageLocationFromEntry(
      resolvedModulePath,
      "@earendil-works/pi-tui",
    );
    if (
      !packageLocation ||
      !SUPPORTED_PI_TUI_VERSION.test(packageLocation.version)
    ) {
      return undefined;
    }
    ...
    if (!prototype || !discoverLayoutMethod(prototype as PatchPrototype))
      return undefined;
...
> {
  ...
  return entrypoint
    ? discoverAccentRailLayoutPatchTargetFromEntrypoint(entrypoint)
    : undefined;
}
